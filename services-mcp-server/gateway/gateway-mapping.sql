-- =============================================================================
-- gateway-mapping.sql — registro DECLARATIVO (pull-based) do services-mcp no
...
-- prefixo 'platform-' → 'services-mcp', e audience = 'mcp:services-mcp'. O PEP do
-- sidecar (MCP_TWIN_AUDIENCE) re-verifica o inner token EXATAMENTE nessa audiência.
-- name_microservice='platform-services-mcp' ⇒ audiência esperada 'mcp:services-mcp'.
...
VALUES
  ('platform-services-mcp', NULL, '/v1/health', 30,
   'mcp_http', 'http://platform-services-mcp:7100', '/mcp/tools/list', '/mcp/tools/call', 'mcp')
...
--     FROM ADMIN_DATAFORALL.GATEWAY_MAPPING WHERE name_microservice='platform-services-mcp';
--   -- audiência derivada esperada: mcp:services-mcp
