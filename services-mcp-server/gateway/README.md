# Registro do services-mcp no MCP Gateway (STD-MCP-001)
...
```
name_microservice = "platform-services-mcp"
  → namespace = removeprefix("platform-") = "services-mcp"
  → inner-token audience = "mcp:services-mcp"
      DEVE == MCP_TWIN_AUDIENCE do sidecar (src/config/settings.py)
```

`MCP_TWIN_AUDIENCE=mcp:services-mcp` e `name_microservice=platform-services-mcp`
...
- `mcp_url=http://platform-services-mcp:7100` (DNS do container = name_microservice).
- `health_path=/v1/health` (sem token), `tools_list_path=/mcp/tools/list`, `tools_call_path=/mcp/tools/call`.
- `exemptTools=[]` — services-mcp não expõe tool tokenless; toda execução exige inner token válido (`mcp:services-mcp`, `jti`).
- 32 tools (registry, portmap, discovery, composite, gateway, launch, env, infra, brokers, logs); cada uma emite
  `capability`, `required_scope` (`services-mcp:<tipo>:<read|write>`), `resource_type` e `data_domain` em `/mcp/tools/list`.
