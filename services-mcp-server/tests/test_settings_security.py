...
from src.config.settings import NAMESPACE, ServicesSettings, get_settings, load_secret
...
# ── namespace / audiência default ─────────────────────────────────────────────
def test_namespace_and_default_audience():
    assert NAMESPACE == "services-mcp"
    assert ServicesSettings().mcp_twin_audience == "mcp:services-mcp"
...
def test_enforce_ok_local_defaults():
    ServicesSettings().enforce_security_invariants()  # local não exige JWKS/admin
...
def test_load_secret_no_vault_returns_env(monkeypatch):
    ...
    assert load_secret("services-mcp/db_password", "from-env") == "from-env"
...
def test_load_secret_vault_import_failure_degrades(monkeypatch):
    ...
    assert load_secret("services-mcp/db_password", "fallback") == "fallback"
...
def test_load_secret_vault_success(monkeypatch):
    ...
    assert load_secret("services-mcp/db_password", "fallback") == "from-vault"
...
# ── resolução das senhas (tenant + admin) via model_validator usa load_secret ──
def test_settings_resolves_secrets_via_vault(monkeypatch):
    ...
    s = ServicesSettings(DB_PASSWORD="ignored", ADMIN_DB_PASSWORD="ignored")
