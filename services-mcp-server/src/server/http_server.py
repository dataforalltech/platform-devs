...
def create_services_http_server(services_registry):
    ...
    @app.get("/health")
    async def health():
        return {"status": "ok", "service": "services-mcp", "port": 8001}

    # ========== Services ==========

    @app.get("/services")
    async def get_services(
        environment: str = Query(None), status: str = Query(None), service_type: str = Query(None)
    ):
        result = endpoints.get_services(
            environment=environment, status=status, service_type=service_type
        )
    ...
    async def post_service_health_check(
        name: str = Path(...), status: str = Query(...), response_time_ms: float = Query(None)
    ):
