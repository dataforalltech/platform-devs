...
def create_deploy_http_server(deploy_manager):
    ...
    async def get_repositories(organization: str = Query(None), status: str = Query(None)):
        result = endpoints.get_repositories(organization=organization, status=status)
    ...
    async def get_repository_acr_images(name: str = Path(...), limit: int = Query(20)):
    ...
    async def post_repository_workflow_run(
        name: str = Path(...), workflow_id: str = Query(...), ref: str = Query(...)
    ):
        result = endpoints.post_repository_workflow_run(name=name, workflow_id=workflow_id, ref=ref)
    ...
    async def get_repository_pull_requests(name: str = Path(...), state: str = Query("open")):
