# Registro do audit-mcp no MCP Gateway (STD-MCP-001)
...
```
name_microservice = "platform-audit-mcp"
  → namespace = removeprefix("platform-") = "audit-mcp"
  → inner-token audience = "mcp:audit-mcp"
      DEVE == MCP_TWIN_AUDIENCE do sidecar (src/config/settings.py)
```

`MCP_TWIN_AUDIENCE=mcp:audit-mcp` e `name_microservice=platform-audit-mcp`
...
- `mcp_url=http://platform-audit-mcp:7100` (DNS do container = name_microservice).
- `health_path=/v1/health` (sem token), `tools_list_path=/mcp/tools/list`, `tools_call_path=/mcp/tools/call`.
- `exemptTools=[]` — audit-mcp não expõe tool tokenless; toda execução exige inner token válido (`mcp:audit-mcp`, `jti`).
