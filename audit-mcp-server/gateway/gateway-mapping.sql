-- =============================================================================
-- gateway-mapping.sql — registro DECLARATIVO (pull-based) do audit-mcp no
...
-- prefixo 'platform-' → 'audit-mcp', e audience = 'mcp:audit-mcp'. O PEP do
-- sidecar (MCP_TWIN_AUDIENCE) re-verifica o inner token EXATAMENTE nessa audiência.
-- name_microservice='platform-audit-mcp' ⇒ audience esperada 'mcp:audit-mcp'.
...
VALUES
  ('platform-audit-mcp', NULL, '/v1/health', 30,
   'mcp_http', 'http://platform-audit-mcp:7100', '/mcp/tools/list', '/mcp/tools/call', 'mcp')
...
--     FROM ADMIN_DATAFORALL.GATEWAY_MAPPING WHERE name_microservice='platform-audit-mcp';
--   -- audiência derivada esperada: mcp:audit-mcp
