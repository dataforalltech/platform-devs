...
def test_formatter_emits_json_with_service_and_msg():
    ...
    assert payload["service"] == "audit-mcp"
    assert payload["level"] == "INFO"
    assert payload["logger"] == "audit.test"
...
def test_formatter_includes_correlation_fields_when_present():
    payload = json.loads(
        JsonLogFormatter().format(_record(tenant_id="T-1", tool="run_audit", trace_id="abc"))
    )
    assert payload["tenant_id"] == "T-1"
    assert payload["tool"] == "run_audit"
...
def test_configure_logging_installs_single_json_handler():
    ...
    try:
        configure_logging(AuditSettings(log_level="warning"))
