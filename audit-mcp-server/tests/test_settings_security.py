...
def test_enforce_ok_local_defaults():
    AuditSettings().enforce_security_invariants()  # local não exige JWKS/admin
...
def test_load_secret_no_vault_returns_env(monkeypatch):
    ...
    assert load_secret("audit-mcp/db_password", "from-env") == "from-env"
...
def test_load_secret_vault_import_failure_degrades(monkeypatch):
    ...
    assert load_secret("audit-mcp/db_password", "fallback") == "fallback"
...
def test_load_secret_vault_success(monkeypatch):
    ...
    assert load_secret("audit-mcp/db_password", "fallback") == "from-vault"
...
def test_settings_resolves_secrets_via_vault(monkeypatch):
    ...
    s = AuditSettings(DB_PASSWORD="ignored", ADMIN_DB_PASSWORD="ignored")
