...
``RUNTIME_ENV`` e o Vault-fallback ``load_secret`` (degradação graciosa p/ env,
sem tocar Vault real).
"""
...
from src.config.settings import AuditSettings, load_secret


def _base(**over: object) -> AuditSettings:
    kw: dict[str, object] = {
...
def test_enforce_ok_local_defaults():
    ...
    AuditSettings(pg_password="").enforce_security_invariants()
...
def test_enforce_bad_audience_raises():
    with pytest.raises(RuntimeError, match="STD-SEC-006"):
        _base(MCP_TWIN_AUDIENCE="audit-mcp").enforce_security_invariants()
...
def test_load_secret_no_vault_returns_env(monkeypatch):
    ...
    assert load_secret("audit-mcp/pg_password", "from-env") == "from-env"
...
def test_load_secret_vault_import_failure_degrades(monkeypatch):
    ...
    assert load_secret("audit-mcp/pg_password", "fallback") == "fallback"
...
def test_load_secret_vault_success(monkeypatch):
    ...
    assert load_secret("audit-mcp/pg_password", "fallback") == "from-vault"
...
def test_settings_resolves_pg_password_via_vault(monkeypatch):
    ...
    assert AuditSettings(pg_password="ignored").pg_password == "resolved-pw"
