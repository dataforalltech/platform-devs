...
describe('DevTeam Observatory Tools', () => {
  ...
  it('should list tool names', () => {
    ...
    expect(toolNames).toContain('get_devteam_status');
  ...
  it('should dispatch get_devteam_status', async () => {
    const result = await dispatch('get_devteam_status', {
      devteam_name: 'test-devteam',
    });
    const parsed = JSON.parse(result);
