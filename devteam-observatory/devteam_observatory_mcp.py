#!/usr/bin/env python3
"""
devteam-observatory-mcp - DevTeam Observatory & Monitoring MCP
Port: 7210
"""
import os
import json
...
class PostgresStore:
    def __init__(self, service_name: str = "devteam-observatory"):
...
class DevTeamObservatoryStore:
    def __init__(self):
        self.db = PostgresStore('devteam-observatory')

    def list_metrics(self, devteam_name: Optional[str] = None, metric_type: Optional[str] = None) -> List[Dict[str, Any]]:
        if devteam_name and metric_type:
            return self.db.query('SELECT * FROM metrics WHERE devteam_name = %s AND metric_type = %s ORDER BY recorded_at DESC', [devteam_name, metric_type])
        elif devteam_name:
            return self.db.query('SELECT * FROM metrics WHERE devteam_name = %s ORDER BY recorded_at DESC', [devteam_name])
        return self.db.query('SELECT * FROM metrics ORDER BY recorded_at DESC')

    def create_metric(self, devteam_name: str, metric_type: str, metric_value: Optional[float] = None, tags: Optional[Dict] = None) -> Dict[str, Any]:
        metric_id = f'metric_{uuid4().hex[:12]}'
        now = datetime.now().isoformat()

        self.db.execute(
            'INSERT INTO metrics (id, devteam_name, metric_type, metric_value, tags, recorded_at) VALUES (%s, %s, %s, %s, %s, %s)',
            [metric_id, devteam_name, metric_type, metric_value, json.dumps(tags) if tags else None, now]
        )

        return {'id': metric_id, 'devteam_name': devteam_name, 'metric_type': metric_type, 'metric_value': metric_value, 'recorded_at': now}
...
app = FastAPI(title="devteam-observatory-mcp", version="1.0.0")
store = DevTeamObservatoryStore()
...
async def init(r: MCPRequest) -> MCPResponse:
    return MCPResponse(id=r.id, result={'protocolVersion': '2024-11-05', 'capabilities': {'tools': {}}, 'serverInfo': {'name': 'devteam-observatory-mcp'}})
...
async def call_tool(r: MCPRequest) -> MCPResponse:
    try:
        ...
        if r.params.get('name') == 'list_metrics':
            result = store.list_metrics(args.get('devteam_name'), args.get('metric_type'))
