...
The DevTeam Observatory provides real-time visibility into the health, workload, and quality of the entire DevTeam ecosystem. It aggregates metrics from all MCPs and provides dashboards, trends, and alerts.
...
### 2. DevTeam Workload
**Tracks:**
- Capacity per DevTeam (Product-Manager, Architecture, BackendDevTeam, etc.)
- Cycle time per DevTeam (avg time from assigned to completed)
...
- DevTeam utilization heatmap (peak hours, idle time)
- Queue depth per DevTeam
...
### get_devteam_workload()
Get capacity and cycle time per DevTeam.

**Output:**
```typescript
{
  devteam_capacity: Record<string, {assigned: number; capacity: number; utilization: number}>;
...
```typescript
{
  ...
  utilization_heatmap: Record<string, Record<string, number>>;  // devteam → hour → utilization
}
```
...
### report_metric(metric_type, values, devteam_name?)
DevTeam report metrics (called by DevTeam agents).
...
```typescript
{
  ...
  devteam_name?: string;                // which DevTeam reporting
}
```
...
- **Consumed by**: Management dashboards, DevTeam status displays
