...
export class ObservatoryStore {
  ...
  private initializeTables(): void {
    ...
    this.db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_metrics_devteam ON metrics(devteam_name);
  ...
  recordMetric(metric: {
    ...
    const stmt = this.db.prepare(`
      INSERT INTO metrics (id, devteam_name, metric_type, feature_id, values)
      VALUES (?, ?, ?, ?, ?)
    `);
    stmt.run(id, metric.devteam_name, metric.metric_type, metric.feature_id || null, JSON.stringify(metric.values));
    return id;
  }

  getMetrics(devteamName?: string, metricType?: string, limit: number = 100): any[] {
    let stmt;
    if (devteamName && metricType) {
      stmt = this.db.prepare(`
        SELECT * FROM metrics
        WHERE devteam_name = ? AND metric_type = ?
        ORDER BY timestamp DESC LIMIT ?
      `);
      return stmt.all(devteamName, metricType, limit) as any[];
    } else if (devteamName) {
      stmt = this.db.prepare(`
        SELECT * FROM metrics
        WHERE devteam_name = ?
        ORDER BY timestamp DESC LIMIT ?
      `);
      return stmt.all(devteamName, limit) as any[];
    } else {
      stmt = this.db.prepare(`
        SELECT * FROM metrics
        ORDER BY timestamp DESC LIMIT ?
