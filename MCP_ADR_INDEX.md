...
| [ADR-018](ADR-018-DEVTEAM-GUARDIAN.md) | **DevTeam Guardião** — domínio `guardian` no devteam-mcp: o hub de governança do `platform-service-template` (5 camadas P/ADR/STD/ARCH/RUNBOOK + perfis/conformance) vira software normalizado (tabelas+CRUD versionado, escopo global×projeto, sem JSON-blob); guardião = registro+política (ingere gates, gerencia waiver/conformidade), migração markdown→DB com paridade `validate_hub.py` |
