-- =============================================================================
-- gateway-mapping.sql — registro DECLARATIVO (pull-based) do test-mcp no
...
-- prefixo 'platform-' → 'test-mcp', e audience = 'mcp:test-mcp'. O PEP do
-- sidecar (MCP_TWIN_AUDIENCE) re-verifica o inner token EXATAMENTE nessa audiência.
-- name_microservice='platform-test-mcp' ⇒ audience esperada 'mcp:test-mcp'.
...
VALUES
  ('platform-test-mcp', NULL, '/v1/health', 30,
   'mcp_http', 'http://platform-test-mcp:7100', '/mcp/tools/list', '/mcp/tools/call', 'mcp')
...
--     FROM ADMIN_DATAFORALL.GATEWAY_MAPPING WHERE name_microservice='platform-test-mcp';
--   -- audiência derivada esperada: mcp:test-mcp
