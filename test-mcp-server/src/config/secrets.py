...
`load_secret` resolve um segredo (ex.: a senha do PostgreSQL) pela fonte aprovada
para o ambiente. Em ``cloud`` a fonte é o Vault e SÓ o Vault: qualquer falha de
auth/fetch levanta ``SecretResolutionError`` e o serviço não sobe — "a ausência
ou invalidez de uma credencial exigida MUST falhar fechada" (STD-SEC-002, §MUST).
Em ``local`` o Vault é opcional e qualquer falha degrada para env, o "fallback
local controlado" que o STD-SEC-004 permite.
