...
- **CAUSA-RAIZ (confirmada via git):** **imagem `:latest` do ACR desatualizada.** O código atual (`platform-auth` @ `release/1.4.0`, `config.py:314-322`) define `URL_ADMIN`/`URL_IAM` = `http://platform-admin:8000/api/internal` (**direto, NÃO pelo gateway**; o check de senha delega para `POST /api/internal/auth`, autorizado por `require_internal_token`) — com um comentário que descreve *exatamente* este 403 como o motivo. O fix entrou no commit **`f82581e`**. Porém a imagem `:latest` puxada do ACR resolve `settings.URL_ADMIN` para `http://platform-api-gateway:8000/api/v1/admin` (comportamento **anterior** ao fix). Ou seja: o código foi corrigido, mas **a imagem não foi rebuildada**.
- **Correção (root-aligned):** fixar `URL_ADMIN`/`URL_IAM=http://platform-admin:8000/api/internal` no compose do auth (sobrescreve o default velho com o valor que o próprio código documenta como correto; permanece correto após rebuild). **Status:** ✅ bakado (compose do auth). **Correção definitiva:** rebuildar `platform-auth:latest` no ACR.
...
### K10. produto sales — platform-crm-mcp (4 bugs de repo + config) e platform-sales-partners-mcp
- **Contexto:** fechar o produto `sales` (tenant `sales`) exigia subir os 2 sidecars MCP. O **sales-partners-mcp** subiu limpo: `build-service.sh platform-sales-partners-mcp platform-sales-partners develop Dockerfile.mcp .` → `docker compose up -d --no-deps` → migrar (`alembic upgrade head` no tenant `sales`, revisões 001→008, schema `sales` foi p/ 89 tabelas) → registrar. Porta **7107**, estilo `mcp`. O **crm-mcp** deu trabalho: **4 bugs de repo encadeados + 1 de config + 1 de registro.**
- **O MCP do crm mora em `mcp/`** (pyproject próprio `name=platform-crm-mcp`, `[project.scripts] platform-crm-mcp = "src.server.mcp_server:main"`, server em `mcp/src/server/mcp_server.py`, **240 tools**). Porta **7100**, health `/v1/health`, tools em **`/mcp/tools/list` + `/mcp/tools/call`** (estilo `mcp`).
- **Bugs no `platform-crm` (branch develop), na ordem em que apareceram:**
  1. **Contexto de build errado (é o J6).** O HANDOFF §7.A.1 mandava buildar com contexto `.` (raiz). O `mcp/Dockerfile` faz `COPY pyproject.toml .` / `COPY src/ ./src/` **relativo ao contexto** → com a raiz, copiou o `pyproject.toml`/`src/platform_crm` da **API** (sem o console-script) → wheel **vazio (866 bytes)**; `CMD ["platform-crm-mcp"]` → `exec: "platform-crm-mcp": executable file not found in $PATH`. → **contexto = `mcp`**.
  2. **Builder sem `COPY src/`, sem `git`, sem secret.** Mesmo com contexto certo, o builder original só fazia `COPY pyproject.toml .` + `pip install "."` — sem copiar `src/`, sem `git`, e **sem** `--mount=type=secret,id=github_token`. → adicionar `COPY src/ ./src/`, `apt-get install git`, e o bloco `--mount=type=secret` + `git config --global url.https://x-access-token:$TOKEN@github.com/.insteadOf git+ssh://...` (igual ao `Dockerfile` principal do serviço).
  3. **`mcp/pyproject.toml` sem `allow-direct-references`.** A dep `platform-governance @ git+ssh://…@feature/dtr-twin-gateway` é *direct reference*; hatchling recusa: `Dependency #7 ... cannot be a direct reference unless tool.hatch.metadata.allow-direct-references is set to true`. → adicionar `[tool.hatch.metadata]\nallow-direct-references = true`.
  4. **Falta `platform-core-lib`.** `platform-governance` importa `platform_core.base_client` → `ModuleNotFoundError: No module named 'platform_core'`. O `mcp/pyproject.toml` declara só `platform-governance`, não a `platform-core-lib` que a governance precisa (o sales-partners-mcp instala explícito via `requirements.mcp.private.txt`). → instalar `platform-core-lib@v0.3.0` junto no `pip install` do builder.
- **Bug de config (compose — `deploy/services/platform-crm/`):** o `Settings` (pydantic-settings) usa `env_prefix="MCP_SERVICE_"` + **nome do campo** (`service_base_url`, `service_token`) → os env REALMENTE lidos são **`MCP_SERVICE_SERVICE_BASE_URL`** e **`MCP_SERVICE_SERVICE_TOKEN`** (dobrado), apesar da docstring do campo dizer `MCP_SERVICE_TOKEN`. O compose mandava `MCP_SERVICE_CRM_URL`/`MCP_SERVICE_INTERNAL_API_TOKEN` → `pydantic ValidationError: service_token Field required, input_value={}`. → compose corrigido p/ os nomes dobrados (confirmado com `docker run -e MCP_SERVICE_SERVICE_TOKEN=x ... Settings()`).
- **Bug de registro:** a openapi do crm-mcp expõe **`/mcp/tools/list` + `/mcp/tools/call`** (estilo `mcp`) — NÃO `/v1/tools` (o comentário do compose dizia "/v1/tools"). Registrado como `v1` primeiro → front-door deu `404 /v1/tools`; corrigido p/ `mcp`.
- **Status:** ✅ crm-mcp e sales-partners-mcp **Up (healthy)**; front-door **1022 tools / 26 services** (era 772/24); **48 containers**. Compose + `register-mcp-backends.sh` **bakados** no `deploy/` (platform-devs). Correção do `mcp/Dockerfile` + `mcp/pyproject.toml` (bugs 1-4) foi feita no **clone da box** e a imagem está no ACR (build manual com BuildKit), mas **PENDENTE de PR no repo `platform-crm`** p/ reprodutibilidade (após o PR: `build-service.sh platform-crm-mcp platform-crm develop mcp/Dockerfile mcp`, contexto `mcp`). **Nota de token:** `MCP_SERVICE_SERVICE_TOKEN` recebe o `INTERNAL_API_TOKEN` (boota/healthy/lista tools); a semântica de auth das *chamadas* às tools ao CRM API fica p/ validar.
