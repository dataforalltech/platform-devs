...
| `partner.dataforall.tech` | **`platform-sales-partners-frontend`** (branch **`develop`**) | platform-sales-partners | **`partner`** ✅ provisionado | 🟡 **frontend NO AR, mas LOGIN NAO FUNCIONA.** SPA (`Console do Parceiro`) roteado por Host; tenant `partner` completo (188 tbls: IAM+auth+servicos via `onboard-tenant.sh`); superadmin `admin@dataforall.tech`/`admin123` (só p/ auth PADRAO). **BLOQUEIO (testado no browser):** o console faz `POST /api/v1/public/**partner-auth**/login` → **404**. Causas: (1) `platform-sales-partners` (REST) **nao esta no GATEWAY_MAPPING** (so `platform-auth`=/auth + MCPs); (2) o backend **nao implementa `partner-auth/login`** — os routers reais sao `/partner`, `/public/partner-invite`, `/admin/*`, `/internal/*` (nenhum de auth de parceiro). O `partner-auth` e uma **feature de backend faltando** (dev). Registrar o sales-partners no gateway (path_prefix `/partner`) destrava dados pos-login, mas nao o login. |
