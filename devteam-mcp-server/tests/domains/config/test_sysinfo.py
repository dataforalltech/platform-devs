# Portado de `config-mcp-server/tests/test_sysinfo.py` pelo fan-out de testes.
#
...
import src.domains.config.knowledge.sysinfo as sysinfo
from src.domains.config.tools.sysinfo_tool import get_physical_info
...
class TestGetPhysicalInfoTool:
    ...
    async def test_failure_returns_error(self, monkeypatch):
        ...
        monkeypatch.setattr("src.domains.config.tools.sysinfo_tool.collect_physical_info", boom)
        result = await get_physical_info()
