"""Settings do devteam-mcp (sidecar mcp_http, Model C) — server AGREGADOR.

`devteam-mcp` consolida os ~20 backends DevTeam (personas + system) num único server:
1 image, 1 deploy, 1 audiência. O gateway central (`platform-mcp`) continua sendo o
ponto único pro cliente; muda só o backend. Cada domínio (architecture, backend, ...)
vira um sub-pacote em `src/domains/<domain>/` com um `plugin.register()`; estas Settings
são COMPARTILHADAS por todos os domínios (1 pool por tenant, 1 inner token, 1 aud).
...
A persistência de cada domínio roda 100% sobre o ORM canônico
(`platform_database.orm`), **tenant-scoped e dual-db**: credencial-zero (ORM-H-12) — o
serviço só conhece o `tenant_id`; a credencial do banco do tenant vem de
...
admin que lê PLATFORMS) — o mesmo objeto é passado a `orm.configure()` no boot.
...
# namespace canônico = name_microservice ('platform-devteam-mcp') menos o prefixo
# 'platform-'. A audiência do inner token DEVE ser exatamente mcp:<namespace>.
NAMESPACE = "devteam-mcp"
...
class DevteamSettings(BaseSettings):
    """Settings do devteam-mcp: gateway (Model C) + backend MySQL (ORM canônico, dual-db)."""
    ...
    def _resolve_secrets(self) -> DevteamSettings:
...
@lru_cache(maxsize=1)
def get_settings() -> DevteamSettings:
