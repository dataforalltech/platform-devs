"""Plugin do domínio *product-manager* — contrato ``register()`` do server consolidado.
...
``register()`` devolve:
  * ``name``       — a chave do domínio (``product-manager``); o agregador acha o domínio
                     por longest-prefix-match (``name.startswith(f"{DOMAIN}_")``), robusto a
                     chaves com hífen (``product-manager``, ``product-owner``, ``ai-governance``).
  * ``schemas``    — o ``catalog._TOOL_SCHEMAS`` com as CHAVES prefixadas
                     (``save_feature_spec`` → ``product-manager_save_feature_spec``). Cada meta
                     já traz capability (``devteam-mcp.product-manager_<op>``), required_scope
                     (``product-manager:<res>:<ação>``), resource_type, data_domain, desc e
                     schema (type=object).
...
# Prefixo de tool do domínio (``product-manager_``). O agregador descobre o domínio por
...
async def _dispatch(name: str, args: dict[str, Any], session: Any) -> dict[str, Any]:
    ...
    (``product-manager_save_feature_spec``) e a sessão tenant-scoped; construímos aqui a
    ``ProductManagerStore`` (single-store) e o catálogo roteia por nome de op sem prefixo
    (``save_feature_spec``) — tolera ambas as formas (idempotente se o prefixo faltar)."""
