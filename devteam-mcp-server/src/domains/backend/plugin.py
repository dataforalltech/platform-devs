"""Plugin do domínio *backend* — contrato ``register()`` do server consolidado.
...
``register()`` devolve:
  * ``name``       — a chave do domínio (``backend``); o agregador acha o domínio
                     por longest-prefix-match (``name.startswith(f"{DOMAIN}_")``).
  * ``schemas``    — o ``catalog._TOOL_SCHEMAS`` com as CHAVES prefixadas
                     (``save_api_contract`` → ``backend_save_api_contract``). Cada meta já
                     traz capability (``devteam-mcp.backend_<op>``), required_scope
                     (``backend:<res>:<ação>``), resource_type, data_domain, desc e
                     schema (type=object).
  * ``dispatch``   — ``async fn(name, args, session)``: recebe o nome JÁ prefixado e a
                     sessão tenant-scoped; CONSTRÓI a ``BackendStore`` sobre a sessão,
...
# Prefixo de tool do domínio (``backend_``). O agregador descobre o domínio por
...
async def _dispatch(name: str, args: dict[str, Any], session: Any) -> dict[str, Any]:
    ...
    (``backend_save_api_contract``) e a sessão tenant-scoped; construímos aqui a
    ``BackendStore`` (single-store) e o catálogo roteia por nome de op sem prefixo
    (``save_api_contract``) — tolera ambas as formas (idempotente se o prefixo faltar)."""
