"""Catálogo de tools + dispatcher do domínio *services* (consolidado no devteam-mcp).

Fatia "de negócio" do antigo `services-mcp-server/src/server/mcp_server.py`: mantém
intactos o ``_TOOL_SCHEMAS`` (schemas + ``SCOPE_FOR_TOOL``/injeção dos metadados de
policy) e o roteamento (``_dispatch``). O que ficou de FORA é o boot/serve/segurança
(FastAPI, PEP inner-token, stdio, tenant plumbing) — responsabilidade do AGREGADOR
...
Diferenças vs. o server-fonte (strangler — fiel, sem inventar comportamento):
  * ``capability`` passa a ``devteam-mcp.services_<op>`` (namespace do server consolidado +
    nome de tool já prefixado pelo domínio — o gateway não deriva por nome).
  * ``required_scope`` passa a ``services:<recurso>:<ação>`` — PRESERVA o least-privilege
    por-tool (``resource_type``/``data_domain`` inalterados), só troca o antigo prefixo de
    namespace ``services-mcp`` pelo domínio canônico ``services``.
  * As CHAVES de ``_TOOL_SCHEMAS`` continuam os nomes de op SEM prefixo (``register_service``);
    o ``plugin.register()`` prefixa (``services_register_service``) e o ``plugin.dispatch``
    retira o prefixo antes de chamar ``dispatch`` daqui — o roteamento fica byte-a-byte.
  * O ``_run_tool`` do fonte (schema-ensure + ``for_tenant`` + ``ServiceStore``) some: o
    agregador já garante o schema e abre a sessão tenant-scoped; a ``dispatch`` daqui recebe
    a ``ServiceStore`` da sessão. ``settings`` é dep de RUNTIME construída aqui
    (``get_settings()``) — o dispatcher usa apenas os defaults de tuning
    (``docker_timeout``/``health_timeout``), ausentes do settings compartilhado.
"""
...
for _tool, _meta in _TOOL_SCHEMAS.items():
    ...
    _meta["capability"] = f"devteam-mcp.{DOMAIN}_{_tool}"
    _meta["required_scope"] = f"{DOMAIN}:{_rtype}:{_action_for(_tool)}"
...
# ── Dispatch público (constrói a dep de RUNTIME `settings` e delega ao roteamento) ──


async def dispatch(name: str, args: dict[str, Any], store: ServiceStore) -> dict[str, Any]:
    ...
    ``settings`` é dep de RUNTIME (``get_settings()``, cacheada): o roteamento do
    server-fonte injeta os defaults de tuning (``docker_timeout``/``health_timeout``) nas
    tools quando o arg é omitido. Não abre sessão nem garante schema — isso é do agregador."""
    settings = get_settings()
    return await _dispatch(name, args, store, settings)
