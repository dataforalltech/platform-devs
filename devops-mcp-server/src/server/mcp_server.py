"""DevOps MCP Server — Streamable HTTP (SDK oficial) + auth (padrão da plataforma).
...
# ── Configuração (env) ──────────────────────────────────────────────────── #
RESOURCE = os.getenv("DEVOPS_RESOURCE", "http://localhost:7121/mcp")
...
# Escopos:
#   devops:read  → consulta/status/análise (não gera artefatos)
#   devops:write → geração de config/runbook/manifests (produz artefato como dict)
# Todas as tools de geração apenas montam e retornam um dict de artefato — não
# executam ações de infra irreversíveis (deploy/restart/destroy). Portanto
# sensitive=False em todas. Se no futuro forem adicionadas tools que aplicam
# mudanças reais na infra, marque sensitive=True e use devops:write (ou devops:deploy).
TOOL_REGISTRY: dict[str, tuple[Callable, str, bool]] = {
    # Leitura / status
    "status": (stub_tool, "devops:read", False),
    # Geração de artefatos (manifests, Dockerfile, pipelines, charts)
    "generate_kubernetes_manifest": (generate_kubernetes_manifest, "devops:write", False),
    "generate_dockerfile": (generate_dockerfile, "devops:write", False),
    "generate_github_actions_pipeline": (generate_github_actions_pipeline, "devops:write", False),
    "generate_helm_chart": (generate_helm_chart, "devops:write", False),
}

SCOPES_SUPPORTED = ["devops:read", "devops:write"]
...
def _scope_for_request(method: str, tool: str | None) -> str | None:
    ...
    return entry[1] if entry else "devops:read"


def build_mcp() -> FastMCP:
    mcp = FastMCP(name="devops-mcp", instructions=SYSTEM_PROMPT, stateless_http=(os.getenv("MCP_STATELESS", "0") == "1"))
...
def build_app(validators: list[Callable] | None = None):
    ...
    async def health(_req: Request):
        return JSONResponse({"status": "ok", "server": "devops-mcp"})
...
def main() -> None:
    import uvicorn
    uvicorn.run(build_app(), host="0.0.0.0", port=int(os.getenv("MCP_PORT", "7121")))
