# Registro do devops-mcp no MCP Gateway (STD-MCP-001)
...
```
name_microservice = "platform-devops-mcp"
  → namespace = removeprefix("platform-") = "devops-mcp"
  → inner-token audience = "mcp:devops-mcp"
      DEVE == MCP_TWIN_AUDIENCE do sidecar (src/config/settings.py)
```

`MCP_TWIN_AUDIENCE=mcp:devops-mcp` e `name_microservice=platform-devops-mcp`
...
- `mcp_url=http://platform-devops-mcp:7100` (DNS do container = name_microservice).
- `health_path=/v1/health` (sem token), `tools_list_path=/mcp/tools/list`, `tools_call_path=/mcp/tools/call`.
- `exemptTools=["status"]` (tokenless); execução das demais exige inner token válido (`mcp:devops-mcp`, `jti`).
