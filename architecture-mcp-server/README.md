...
Architecture is an MCP (Model Context Protocol) server that provides 18 tools for architectural analysis, design, and decision-making. It helps teams:
...
```json
{
  "architecture-mcp-server": {
    "command": "bash",
    "args": ["-c", "cd /home/dev/repos/platform-devs/architecture-mcp-server && node dist/server.js"]
  }
}
```
...
Architecture uses SQLite (WAL mode) with 4 tables:
...
```
ARCHITECTURE_DB_PATH=/tmp/architecture.db
...
Architecture's system prompt emphasizes:
...
The prompt is exposed as MCP resource: `architecture_system_prompt`
...
For integration with Claude Code, reference the system prompt `architecture_system_prompt` for detailed guidance on architecture-driven decision-making.
