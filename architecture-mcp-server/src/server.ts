...
import { ArchitectureStore } from './db/store.js';
import { getSettings } from './config/settings.js';
import { getArchitecturePrompt } from './prompts/architecturePrompt.js';
...
async function handleListResources() {
  return {
    resources: [
      {
        uri: 'architecture_system_prompt',
        name: 'Architecture System Prompt',
        description: 'System prompt for Architecture agent specializing in software architecture design',
        mimeType: 'text/plain',
      },
    ],
  };
...
async function handleReadResource(request: {
  ...
  if (uri === 'architecture_system_prompt') {
    return {
      contents: [
        {
          uri: 'architecture_system_prompt',
          mimeType: 'text/plain',
          text: getArchitecturePrompt(),
