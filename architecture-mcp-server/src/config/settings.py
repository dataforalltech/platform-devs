"""Settings do architecture-mcp (sidecar mcp_http, Model C).
...
`architecture-mcp` é uma persona **stateful**: o agente gera o conteúdo (propostas de
arquitetura, modelos C4, blueprints de solução, artefatos) e as tools o **persistem**.
A persistência roda 100% sobre o ORM canônico (`platform_database.orm`), **tenant-
...
class ArchitectureSettings(BaseSettings):
    """Settings do architecture-mcp: gateway (Model C) + backend MySQL (ORM canônico, dual-db)."""
    ...
    def _resolve_secrets(self) -> ArchitectureSettings:
...
@lru_cache(maxsize=1)
def get_settings() -> ArchitectureSettings:
