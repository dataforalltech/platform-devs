-- =============================================================================
-- gateway-mapping.sql — registro DECLARATIVO (pull-based) do architecture-mcp no
...
-- prefixo 'platform-' → 'architecture-mcp', e audience = 'mcp:architecture-mcp'. O PEP do
-- sidecar (MCP_TWIN_AUDIENCE) re-verifica o inner token EXATAMENTE nessa audiência.
-- name_microservice='platform-architecture-mcp' ⇒ audience esperada 'mcp:architecture-mcp'.
...
VALUES
  ('platform-architecture-mcp', NULL, '/v1/health', 30,
   'mcp_http', 'http://platform-architecture-mcp:7100', '/mcp/tools/list', '/mcp/tools/call', 'mcp')
...
--     FROM ADMIN_DATAFORALL.GATEWAY_MAPPING WHERE name_microservice='platform-architecture-mcp';
--   -- audiência derivada esperada: mcp:architecture-mcp
