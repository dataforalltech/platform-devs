# Registro do architecture-mcp no MCP Gateway (STD-MCP-001)
...
```
name_microservice = "platform-architecture-mcp"
  → namespace = removeprefix("platform-") = "architecture-mcp"
  → inner-token audience = "mcp:architecture-mcp"
      DEVE == MCP_TWIN_AUDIENCE do sidecar (src/config/settings.py)
```

`MCP_TWIN_AUDIENCE=mcp:architecture-mcp` e `name_microservice=platform-architecture-mcp`
...
- `mcp_url=http://platform-architecture-mcp:7100` (DNS do container = name_microservice).
- `health_path=/v1/health` (sem token), `tools_list_path=/mcp/tools/list`, `tools_call_path=/mcp/tools/call`.
- `exemptTools=["status"]` (tokenless); execução das demais exige inner token válido (`mcp:architecture-mcp`, `jti`).
