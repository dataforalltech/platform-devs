...
**Product-Owner** — Product Owner specialist agent for backlog management, refinement, and agile delivery.

## Overview

Product-Owner transforms business demands, product vision, and functional requirements into clear, prioritized, testable, and deployment-ready backlogs.

## Capabilities

Product-Owner specializes in:
...
- `PRODUCT_OWNER_DB_PATH` — SQLite database path (default: `/tmp/product-owner.db`)
- `PRODUCT_OWNER_LOG_LEVEL` — Log level: debug|info|warn|error (default: `info`)
...
Product-Owner is consumed via MCP protocol as a language model tool provider. Agents call tools via:
...
└── prompts/
    └── product-ownerPrompt.ts  — System prompt resource
```

## MCP Features

- **Tools**: 17 specialized backlog & delivery tools
- **Resources**: System prompt (`product-owner_system_prompt`)
- **Capabilities**: Tools, resources
- **Transport**: Stdio (MCP SDK)

---

**Product-Owner: transforma ideia solta em backlog pronto para sprint.**
