#!/usr/bin/env python3
"""
product-owner-mcp - product-owner MCP Service
Port: 7207
"""
import os
import json
...
class PostgresStore:
    def __init__(self, service_name: str = "product-owner"):
...
class ProductOwnerStore:
    def __init__(self):
        self.db = PostgresStore('product-owner')

    def list_epics(self) -> List[Dict[str, Any]]:
        return self.db.query('SELECT * FROM epics ORDER BY created_at DESC')

app = FastAPI(title="product-owner-mcp", version="1.0.0")
...
TOOLS = {'list_epics': {'name': 'list_epics', 'description': 'List epics', 'inputSchema': {'type': 'object'}}}

@app.post("/mcp/initialize")
async def init(r: MCPRequest) -> MCPResponse:
    return MCPResponse(id=r.id, result={'protocolVersion': '2024-11-05', 'capabilities': {'tools': {}}, 'serverInfo': {'name': 'product-owner-mcp'}})
...
async def call_tool(r: MCPRequest) -> MCPResponse:
    try:
        if r.params.get('name') == 'list_epics':
            result = store.list_epics()
        else:
...
if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 7207))
