"""Settings do product-owner-mcp (sidecar mcp_http, Model C).
...
`product-owner-mcp` é uma persona **stateful**: o agente gera o conteúdo (user
stories, escopo de MVP, visão de produto, personas, itens de backlog, artefatos de
PO) e as tools o **persistem**. A persistência roda 100% sobre o ORM canônico
...
`DBSettings` (`DB_*`, fallback compartilhado) e `AdminDBSettings` (`ADMIN_DB_*`,
...
# namespace canônico = name_microservice ('platform-product-owner-mcp') menos o
# prefixo 'platform-'. A audiência do inner token DEVE ser exatamente mcp:<namespace>.
NAMESPACE = "product-owner-mcp"
...
class ProductOwnerSettings(BaseSettings):
    """Settings do product-owner-mcp: gateway (Model C) + backend MySQL (ORM canônico, dual-db)."""
    ...
    def _resolve_secrets(self) -> ProductOwnerSettings:
...
Settings = ProductOwnerSettings


@lru_cache(maxsize=1)
def get_settings() -> ProductOwnerSettings:
