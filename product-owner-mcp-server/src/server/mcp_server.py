...
_TOOL_SCHEMAS: dict[str, dict[str, Any]] = {
    ...
    "map_product_risks": {
        "description": "Classifica riscos de produto nas 4 categorias de Cagan (valor, usabilidade, viabilidade, feasibility).",  # noqa: E501
...
def _dispatch(name: str, args: dict[str, Any]) -> dict[str, Any]:
    ...
    if name == "calculate_rice_score":
        return calculate_rice_score(
            reach=args.get("reach", 0.0),
            impact=args.get("impact", 0.0),
            confidence=args.get("confidence", 0.0),
            effort=args.get("effort", 0.0),
            feature=args.get("feature"),
        )
    if name == "prioritize_backlog":
        return prioritize_backlog(items=args.get("items", []), framework=args.get("framework", "RICE"))
    if name == "map_product_risks":
        return map_product_risks(feature=args.get("feature", ""), risks=args.get("risks"))
    ...
    if name == "define_product_vision":
        return define_product_vision(
            ...
            differentiator=args.get("differentiator"),
    ...
    if name == "generate_feature_spec":
        return generate_feature_spec(
            feature=args.get("feature", ""),
            problem=args.get("problem"),
            user_value=args.get("user_value"),
    ...
    if name == "generate_go_to_market_brief":
        return generate_go_to_market_brief(
            product=args.get("product", ""),
            target_segment=args.get("target_segment", ""),
            value_proposition=args.get("value_proposition", ""),
            channels=args.get("channels"),
            launch_date=args.get("launch_date"),
        )
    ...
    if name == "generate_handoff_to_design":
        return generate_handoff_to_design(
            feature=args.get("feature", ""),
            user_journeys=args.get("user_journeys"),
            personas=args.get("personas"),
            key_screens=args.get("key_screens"),
        )
