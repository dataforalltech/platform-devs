# Registro do product-owner-mcp no MCP Gateway (STD-MCP-001)
...
```
name_microservice = "platform-product-owner-mcp"
  → namespace = removeprefix("platform-") = "product-owner-mcp"
  → inner-token audience = "mcp:product-owner-mcp"
      DEVE == MCP_TWIN_AUDIENCE do sidecar (src/config/settings.py)
```

`MCP_TWIN_AUDIENCE=mcp:product-owner-mcp` e `name_microservice=platform-product-owner-mcp`
...
- `mcp_url=http://platform-product-owner-mcp:7100` (DNS do container = name_microservice).
- `health_path=/v1/health` (sem token), `tools_list_path=/mcp/tools/list`, `tools_call_path=/mcp/tools/call`.
- `exemptTools=[]` (nenhuma tool tokenless — a liveness é o `/v1/health`); toda execução exige inner token válido (`mcp:product-owner-mcp`, `jti`).
