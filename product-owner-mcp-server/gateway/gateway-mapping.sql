-- =============================================================================
-- gateway-mapping.sql — registro DECLARATIVO (pull-based) do product-owner-mcp no
...
-- prefixo 'platform-' → 'product-owner-mcp', e audience = 'mcp:product-owner-mcp'. O PEP do
-- sidecar (MCP_TWIN_AUDIENCE) re-verifica o inner token EXATAMENTE nessa audiência.
-- name_microservice='platform-product-owner-mcp' ⇒ audience esperada 'mcp:product-owner-mcp'.
...
VALUES
  ('platform-product-owner-mcp', NULL, '/v1/health', 30,
   'mcp_http', 'http://platform-product-owner-mcp:7100', '/mcp/tools/list', '/mcp/tools/call', 'mcp')
...
--     FROM ADMIN_DATAFORALL.GATEWAY_MAPPING WHERE name_microservice='platform-product-owner-mcp';
--   -- audiência derivada esperada: mcp:product-owner-mcp
