"""Fixtures da Suíte Canônica do product-owner-mcp (§16 — PA-01 / FID-02).

**Banco REAL, nunca mockado** (FID-02): os testes do store/servidor rodam contra um
MySQL 8.x real com **2 tenants** (banco-por-tenant), exercitando o caminho canônico
...
from src.config.settings import ProductOwnerSettings  # noqa: E402
...
from src.db.store import ProductOwnerStore  # noqa: E402
...
TENANT_A = "product_owner_test_a"
TENANT_B = "product_owner_test_b"

# Todas as tabelas do persona (ordem estável p/ TRUNCATE no setup dos fixtures).
_TABLES = (
    USER_STORIES_TABLE,
    MVP_SCOPES_TABLE,
    PRODUCT_VISIONS_TABLE,
...
def _test_settings(**over: Any) -> ProductOwnerSettings:
    ...
    kw.update(over)
    return ProductOwnerSettings(**kw)
...
async def _make_store(tenant_id: str) -> tuple[ProductOwnerStore, Any]:
    ...
    return ProductOwnerStore(TenantSession(pool, tenant_id)), token
