# Registro do config-mcp no MCP Gateway (STD-MCP-001)
...
```
name_microservice = "platform-config-mcp"
  → namespace = removeprefix("platform-") = "config-mcp"
  → inner-token audience = "mcp:config-mcp"
      DEVE == MCP_TWIN_AUDIENCE do sidecar (src/config/settings.py)
```

`MCP_TWIN_AUDIENCE=mcp:config-mcp` e `name_microservice=platform-config-mcp`
...
- `mcp_url=http://platform-config-mcp:7100` (DNS do container = name_microservice).
- `health_path=/v1/health` (sem token), `tools_list_path=/mcp/tools/list`, `tools_call_path=/mcp/tools/call`.
- `exemptTools=["status"]` (tokenless); execução das demais exige inner token válido (`mcp:config-mcp`, `jti`).
- `excludeTools=["get_credential", "set_credential_secure"]` — denylist fail-safe (CI-7):
  `get_credential` devolve o segredo em claro (risco de exfiltração p/ o agente) e
  `set_credential_secure` depende do TTY (getpass) do processo — nenhum dos dois deve
  sair pelo gateway. Fluxos service-to-service de segredo usam o canal interno/S2S.
