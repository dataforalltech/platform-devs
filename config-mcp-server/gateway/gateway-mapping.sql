-- =============================================================================
-- gateway-mapping.sql — registro DECLARATIVO (pull-based) do config-mcp no
...
-- prefixo 'platform-' → 'config-mcp', e audience = 'mcp:config-mcp'. O PEP do
-- sidecar (MCP_TWIN_AUDIENCE) re-verifica o inner token EXATAMENTE nessa audiência.
-- name_microservice='platform-config-mcp' ⇒ audience esperada 'mcp:config-mcp'.
...
VALUES
  ('platform-config-mcp', NULL, '/v1/health', 30,
   'mcp_http', 'http://platform-config-mcp:7100', '/mcp/tools/list', '/mcp/tools/call', 'mcp')
...
--     FROM ADMIN_DATAFORALL.GATEWAY_MAPPING WHERE name_microservice='platform-config-mcp';
--   -- audiência derivada esperada: mcp:config-mcp
