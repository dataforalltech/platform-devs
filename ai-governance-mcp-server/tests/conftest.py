"""Fixtures da Suíte Canônica do ai-governance-mcp (§16 — PA-01 / FID-02).

Duas famílias de fixtures convivem aqui:

  * ``repo`` — a knowledge-base REAL (Markdown/YAML), carregada uma vez por sessão. As
    tools compute (guidelines/policy/graph/ownership/…) rodam contra ela; testar contra a
    KB do produto detecta drift entre código e knowledge-base.

  * ``stores_a`` / ``stores_b`` — os DOIS stores mutáveis (sugestões + auditoria) sobre um
    **MySQL REAL, nunca mockado** (FID-02), com **2 tenants** (banco-por-tenant),
    exercitando o caminho canônico ``get_pool_for_tenant -> Repository -> dialeto MySQL``.
    O único "duplo" é o ``platform_lookup`` (linha estática no lugar da consulta ao
...
from src.config.settings import NAMESPACE, Settings  # noqa: E402
from src.db.schema import AUDIT_TABLE, SUGGESTIONS_TABLE, ensure_schema  # noqa: E402
from src.db.store import AuditStore, SuggestionStore  # noqa: E402
...
TENANT_A = "ai_governance_test_a"
TENANT_B = "ai_governance_test_b"


# ── knowledge-base compute (read-only, sem DB) ────────────────────────────────
...
async def _make_stores(tenant_id: str) -> tuple[SuggestionStore, AuditStore, Any]:
    ...
    for table in (SUGGESTIONS_TABLE, AUDIT_TABLE):
        await pool.execute(f"TRUNCATE TABLE {table}")
    token = set_tenant_id(tenant_id)
    session = TenantSession(pool, tenant_id)
    return SuggestionStore(session), AuditStore(session), token


@pytest_asyncio.fixture
async def stores_a():
    suggestions, audit, token = await _make_stores(TENANT_A)
    yield suggestions, audit
...
@pytest_asyncio.fixture
async def stores_b():
    suggestions, audit, token = await _make_stores(TENANT_B)
    yield suggestions, audit
    reset_tenant_id(token)
    await close_tenant_pools()
