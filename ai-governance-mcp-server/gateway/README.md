# Registro do ai-governance-mcp no MCP Gateway (STD-MCP-001)
...
```
name_microservice = "platform-ai-governance-mcp"
  → namespace = removeprefix("platform-") = "ai-governance-mcp"
  → inner-token audience = "mcp:ai-governance-mcp"
      DEVE == MCP_TWIN_AUDIENCE do sidecar (src/config/settings.py)
```

`MCP_TWIN_AUDIENCE=mcp:ai-governance-mcp` e `name_microservice=platform-ai-governance-mcp`
...
- `mcp_url=http://platform-ai-governance-mcp:7100` (DNS do container = name_microservice).
- `health_path=/v1/health` (sem token), `tools_list_path=/mcp/tools/list`, `tools_call_path=/mcp/tools/call`.
- `exemptTools=["status"]` (tokenless); execução das demais exige inner token válido (`mcp:ai-governance-mcp`, `jti`).
