...
import { ProductManagerStore } from './db/store.js';
import { getSettings } from './config/settings.js';
import { getProductManagerPrompt } from './prompts/product-managerPrompt.js';

const settings = getSettings();
const store = new ProductManagerStore();
...
async function handleListResources() {
  return {
    resources: [
      {
        uri: 'product-manager_system_prompt',
        name: 'Product-Manager System Prompt',
        description: 'System prompt for Product-Manager agent specializing in product management',
        mimeType: 'text/plain',
      },
    ],
  };
...
async function handleReadResource(request: {
  ...
  if (uri === 'product-manager_system_prompt') {
    return {
      contents: [
        {
          uri: 'product-manager_system_prompt',
          mimeType: 'text/plain',
          text: getProductManagerPrompt(),
