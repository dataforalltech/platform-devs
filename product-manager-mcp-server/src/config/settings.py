...
`product-manager-mcp` é uma persona **stateful**: o agente gera o conteúdo (feature
specs, GTM briefs, release plans, visões de produto, artefatos) e as tools o
**persistem**. A persistência roda 100% sobre o ORM canônico (`platform_database.orm`),
...
class ProductManagerSettings(BaseSettings):
    """Settings do product-manager-mcp: gateway (Model C) + backend MySQL (ORM canônico, dual-db)."""
    ...
    def _resolve_secrets(self) -> ProductManagerSettings:
...
Settings = ProductManagerSettings


@lru_cache(maxsize=1)
def get_settings() -> ProductManagerSettings:
