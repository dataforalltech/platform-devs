# Registro do product-manager-mcp no MCP Gateway (STD-MCP-001)
...
```
name_microservice = "platform-product-manager-mcp"
  → namespace = removeprefix("platform-") = "product-manager-mcp"
  → inner-token audience = "mcp:product-manager-mcp"
      DEVE == MCP_TWIN_AUDIENCE do sidecar (src/config/settings.py)
```

`MCP_TWIN_AUDIENCE=mcp:product-manager-mcp` e `name_microservice=platform-product-manager-mcp`
...
- `mcp_url=http://platform-product-manager-mcp:7100` (DNS do container = name_microservice).
- `health_path=/v1/health` (sem token), `tools_list_path=/mcp/tools/list`, `tools_call_path=/mcp/tools/call`.
- `exemptTools=["status"]` (tokenless); execução das demais exige inner token válido (`mcp:product-manager-mcp`, `jti`).
