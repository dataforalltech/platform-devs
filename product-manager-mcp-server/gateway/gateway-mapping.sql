-- =============================================================================
-- gateway-mapping.sql — registro DECLARATIVO (pull-based) do product-manager-mcp no
...
-- prefixo 'platform-' → 'product-manager-mcp', e audience = 'mcp:product-manager-mcp'. O PEP do
-- sidecar (MCP_TWIN_AUDIENCE) re-verifica o inner token EXATAMENTE nessa audiência.
-- name_microservice='platform-product-manager-mcp' ⇒ audience esperada 'mcp:product-manager-mcp'.
...
VALUES
  ('platform-product-manager-mcp', NULL, '/v1/health', 30,
   'mcp_http', 'http://platform-product-manager-mcp:7100', '/mcp/tools/list', '/mcp/tools/call', 'mcp')
...
--     FROM ADMIN_DATAFORALL.GATEWAY_MAPPING WHERE name_microservice='platform-product-manager-mcp';
--   -- audiência derivada esperada: mcp:product-manager-mcp
