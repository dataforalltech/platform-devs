...
from src.config.settings import ProductManagerSettings, load_secret
...
def test_load_secret_no_vault_returns_env(monkeypatch):
    ...
    assert load_secret("product-manager-mcp/db_password", "from-env") == "from-env"


def test_load_secret_vault_import_failure_degrades(monkeypatch):
    monkeypatch.setenv("VAULT_ADDR", "http://vault.local:8200")
    assert load_secret("product-manager-mcp/db_password", "fallback") == "fallback"
...
def test_load_secret_vault_success(monkeypatch):
    ...
    assert load_secret("product-manager-mcp/db_password", "fallback") == "from-vault"
