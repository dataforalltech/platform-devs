# Registro do frontend-mcp no MCP Gateway (STD-MCP-001)
...
```
name_microservice = "platform-frontend-mcp"
  → namespace = removeprefix("platform-") = "frontend-mcp"
  → inner-token audience = "mcp:frontend-mcp"
      DEVE == MCP_TWIN_AUDIENCE do sidecar (src/config/settings.py)
```

`MCP_TWIN_AUDIENCE=mcp:frontend-mcp` e `name_microservice=platform-frontend-mcp`
...
- `mcp_url=http://platform-frontend-mcp:7100` (DNS do container = name_microservice).
- `health_path=/v1/health` (sem token), `tools_list_path=/mcp/tools/list`, `tools_call_path=/mcp/tools/call`.
- `exemptTools=["status"]` (tokenless); execução das demais exige inner token válido (`mcp:frontend-mcp`, `jti`).
