-- =============================================================================
-- gateway-mapping.sql — registro DECLARATIVO (pull-based) do frontend-mcp no
...
-- prefixo 'platform-' → 'frontend-mcp', e audience = 'mcp:frontend-mcp'. O PEP do
-- sidecar (MCP_TWIN_AUDIENCE) re-verifica o inner token EXATAMENTE nessa audiência.
-- name_microservice='platform-frontend-mcp' ⇒ audience esperada 'mcp:frontend-mcp'.
...
VALUES
  ('platform-frontend-mcp', NULL, '/v1/health', 30,
   'mcp_http', 'http://platform-frontend-mcp:7100', '/mcp/tools/list', '/mcp/tools/call', 'mcp')
...
--     FROM ADMIN_DATAFORALL.GATEWAY_MAPPING WHERE name_microservice='platform-frontend-mcp';
--   -- audiência derivada esperada: mcp:frontend-mcp
