"""Settings do frontend-mcp (sidecar mcp_http, Model C).
...
`frontend-mcp` é uma persona **stateful**: o agente gera o conteúdo (componentes,
páginas, formulários, stories, artefatos de UI) e as tools o **persistem**. A
...
# namespace canônico = name_microservice ('platform-frontend-mcp') menos o
# prefixo 'platform-'. A audiência do inner token DEVE ser exatamente mcp:<namespace>.
NAMESPACE = "frontend-mcp"
...
class FrontendSettings(BaseSettings):
    """Settings do frontend-mcp: gateway (Model C) + backend MySQL (ORM canônico, dual-db)."""
    ...
    def _resolve_secrets(self) -> FrontendSettings:
