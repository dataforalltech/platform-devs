...
NOTA: frontend-mcp é compute-only (gera artefatos de UI a partir dos inputs; não há
...
# é fail-open). Geradores usam verbo :write (produzem artefato); status é :read.
# ─────────────────────────────────────────────────────────────────────────────
_TOOL_SCHEMAS: dict[str, dict[str, Any]] = {
    "status": {
        "description": "Retorna o status real do servidor (nome, versão, nº de tools).",
        "capability": "frontend-mcp.status",
        "required_scope": "frontend-mcp:status:read",
        "resource_type": "status",
        "data_domain": "operational",
        "schema": {"type": "object", "additionalProperties": False, "properties": {}},
    },
    "generate_react_component": {
        "description": "Gera o scaffold de um componente React a partir do nome e da variante.",
        "capability": "frontend-mcp.generate_react_component",
        "required_scope": "frontend-mcp:component:write",
        "resource_type": "component",
        "data_domain": "frontend",
        "schema": {
            "type": "object",
            "additionalProperties": False,
            "properties": {
                "name": {"type": "string", "description": "Nome do componente React."},
                "variant": {
                    "type": "string",
                    "description": "Variante do componente (ex.: functional, class).",
                },
            },
        },
    },
    "generate_nextjs_page": {
        "description": "Gera uma página Next.js (App Router) a partir da rota informada.",
        "capability": "frontend-mcp.generate_nextjs_page",
        "required_scope": "frontend-mcp:page:write",
        "resource_type": "page",
        "data_domain": "frontend",
        "schema": {
            "type": "object",
            "additionalProperties": False,
            "properties": {
                "route": {"type": "string", "description": "Rota da página (ex.: /dashboard)."},
            },
        },
    },
    "generate_storybook_story": {
        "description": "Gera uma story de Storybook (CSF 3.0) para o componente informado.",
        "capability": "frontend-mcp.generate_storybook_story",
        "required_scope": "frontend-mcp:story:write",
        "resource_type": "story",
        "data_domain": "frontend",
        "schema": {
            "type": "object",
            ...
            "properties": {
                "component_name": {
                    "type": "string",
                    "description": "Nome do componente-alvo da story.",
                },
            },
        },
    },
    "generate_form_with_validation": {
        "description": "Gera um formulário com React Hook Form + validação Zod.",
        "capability": "frontend-mcp.generate_form_with_validation",
        "required_scope": "frontend-mcp:form:write",
        "resource_type": "form",
        "data_domain": "frontend",
        "schema": {
            "type": "object",
...
def _dispatch(name: str, args: dict[str, Any]) -> dict[str, Any]:
    ...
    if name == "generate_nextjs_page":
        return generate_nextjs_page(route=args.get("route", "/"))
    if name == "generate_storybook_story":
        return generate_storybook_story(
            component_name=args.get("component_name", "Component"),
        )
    if name == "generate_form_with_validation":
        return generate_form_with_validation(form_name=args.get("form_name", "Form"))
    raise KeyError(name)
...
def build_server() -> tuple[Any, Settings, FastAPI]:
    ...
    _log.info("frontend_mcp_ready tools=%d", len(_TOOL_SCHEMAS))
