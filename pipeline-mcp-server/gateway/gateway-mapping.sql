-- =============================================================================
-- gateway-mapping.sql — registro DECLARATIVO (pull-based) do pipeline-mcp no
...
-- prefixo 'platform-' → 'pipeline-mcp', e audience = 'mcp:pipeline-mcp'. O PEP do
-- sidecar (MCP_TWIN_AUDIENCE) re-verifica o inner token EXATAMENTE nessa audiência.
-- name_microservice='platform-pipeline-mcp' ⇒ audience esperada 'mcp:pipeline-mcp'.
...
VALUES
  ('platform-pipeline-mcp', NULL, '/v1/health', 30,
   'mcp_http', 'http://platform-pipeline-mcp:7100', '/mcp/tools/list', '/mcp/tools/call', 'mcp')
...
--     FROM ADMIN_DATAFORALL.GATEWAY_MAPPING WHERE name_microservice='platform-pipeline-mcp';
--   -- audiência derivada esperada: mcp:pipeline-mcp
