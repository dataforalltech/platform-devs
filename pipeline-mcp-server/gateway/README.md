# Registro do pipeline-mcp no MCP Gateway (STD-MCP-001)
...
```
name_microservice = "platform-pipeline-mcp"
  → namespace = removeprefix("platform-") = "pipeline-mcp"
  → inner-token audience = "mcp:pipeline-mcp"
      DEVE == MCP_TWIN_AUDIENCE do sidecar (src/config/settings.py)
```

`MCP_TWIN_AUDIENCE=mcp:pipeline-mcp` e `name_microservice=platform-pipeline-mcp`
...
- `mcp_url=http://platform-pipeline-mcp:7100` (DNS do container = name_microservice).
- `health_path=/v1/health` (sem token), `tools_list_path=/mcp/tools/list`, `tools_call_path=/mcp/tools/call`.
- `exemptTools=[]` — pipeline-mcp não expõe tool tokenless; toda execução exige inner token
  válido (`mcp:pipeline-mcp`, `jti`). O liveness fica no endpoint `/v1/health` (fora do fluxo de tools).
