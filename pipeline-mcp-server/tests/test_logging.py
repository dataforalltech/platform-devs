...
def test_formatter_emits_json_with_service_and_msg():
    ...
    assert payload["service"] == "pipeline-mcp"
...
def test_formatter_includes_correlation_fields_when_present():
    line = JsonLogFormatter().format(_record(tenant_id="T-1", tool="get_pipeline", trace_id="abc"))
    payload = json.loads(line)
    assert payload["tenant_id"] == "T-1"
    assert payload["tool"] == "get_pipeline"
...
def test_configure_logging_installs_single_json_handler():
    ...
    try:
        configure_logging(PipelineSettings(log_level="warning"))
