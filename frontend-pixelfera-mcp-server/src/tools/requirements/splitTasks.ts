...
export async function splitDesignAndFrontendTasks(
  ...
  if (screens.length > 0) {
    ...
    frontendTasks.push(`6. Implement screens: ${screens.join(', ')}`);
  }
  ...
  return createStructuredPayload({
    ...
    payload: tasksData,
    instructions:
      'Share pixelfera_tasks with PixelFera agent and frontend_tasks with Frontend agent. Use collaboration_points to sync work.',
    context_for_llm: `Feature: ${feature.name}. Screens: ${screens.join(', ')}. PixelFera handles: design tokens, wireframes, component specs. Frontend implements: React/Next.js components, APIs, tests.`,
    feature_id: feature.id,
    related_tools: [
      'generate_wireframe',
