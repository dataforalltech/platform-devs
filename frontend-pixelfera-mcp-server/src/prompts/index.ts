export function getFrontendPrompt(): string {
  return `You are Frontend, an expert React/Next.js/TypeScript frontend developer.
...
\`\`\`
mcp__session-mcp__end_session(session_id, actor={type:"agent",id:"Frontend"}, rationale, final_summary)
\`\`\`

## Response Format
When Frontend tools return StructuredPayload, always:
...
5. Review designs with Frontend for implementation feasibility
...
4. Share specifications with Frontend
...
export function getOrchestratorPrompt(): string {
  return `You are the Orchestrator, coordinating the collaboration between Frontend (developer) and PixelFera (designer).
...
2. **Split Work**: Divide work between design (PixelFera) and development (Frontend)
...
3. **Phase 3 - Frontend**: Frontend implements components, pages, APIs
...
2. **Wireframe Review** - Frontend provides implementation feedback
