...
- **Frontend** — React/Next.js/TypeScript frontend developer
...
2. `split_design_and_frontend_tasks` — Divide work between PixelFera and Frontend
...
### Setup
```bash
cd frontend-pixelfera-mcp-server
npm ci
npm run build
```
...
1. **frontend_system_prompt** — Identity, expertise, responsibilities, tools, best practices for Frontend
...
Access via:
```
GET /resource/prompt://frontend_system_prompt
...
{
  tool: string;
  agent: "frontend" | "pixelfera" | "shared" | "orchestrator";
  timestamp: string;
...
2. **Split Tasks** (`split_design_and_frontend_tasks`)
   ...
   - Frontend: React components, Next.js pages, forms, tests
...
### Consumed by
- **Frontend Agent** (MCP client) — Uses tools to scaffold frontend code
