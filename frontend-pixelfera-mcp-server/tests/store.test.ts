...
import { FrontendPixelferaStore } from '../src/db/store';

describe('FrontendPixelferaStore', () => {
  let store: FrontendPixelferaStore;

  beforeEach(() => {
    store = new FrontendPixelferaStore(':memory:');
  });
  ...
  describe('Components', () => {
    ...
    it('should list components by agent', () => {
      store.createComponent('Button', 'atom', 'frontend', {});
      store.createComponent('Icon', 'atom', 'pixelfera', {});

      const frontendComponents = store.listComponents(undefined, 'frontend');
      expect(frontendComponents.length).toBeGreaterThan(0);
    });
