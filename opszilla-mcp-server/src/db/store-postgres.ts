...
import * as /home/dev/repos/platform-devs/opszilla-mcp-server/src/db/store-postgres.ts from '/home/dev/repos/platform-devs/opszilla-mcp-server/src/db/store-postgres.ts';
...
// ============================================================================
// OpszillaStore — PostgreSQL Primary
// ============================================================================
...
export class OpszillaStore {
  ...
  constructor() {
    ...
    try {
      this.pg = new ZillaPostgresStore('opszilla', postgresConfig);
      this.logger.info('✅ OpszillaStore: PostgreSQL initialized');
    } catch (error) {
      this.logger.error(`❌ OpszillaStore: Failed to initialize: ${error}`);
      throw error;
    }
  }

  private initializeLogger(): any {
    const logsDir = /home/dev/repos/platform-devs/opszilla-mcp-server/src/db/store-postgres.ts.join(process.env.HOME || '/tmp', '.platform', 'logs');
    if (!fs.existsSync(logsDir)) {
      fs.mkdirSync(logsDir, { recursive: true });
    }

    const logFile = /home/dev/repos/platform-devs/opszilla-mcp-server/src/db/store-postgres.ts.join(logsDir, 'opszilla.log');
  ...
  async createTestPlan(data: Omit<Deployment, 'id' | 'created_at' | 'updated_at'>): Promise<Deployment> {
    const id = `tp_${nanoid()}`;
    const now = new Date().toISOString();

    await this.pg.execute(
      `INSERT INTO deployments (id, title, feature, scope, objectives, status, created_at, updated_at)
  ...
  async getTestPlan(id: string): Promise<Deployment | null> {
    const rows = await this.pg.query<Deployment>('SELECT * FROM deployments WHERE id = $1', [id]);
    return rows[0] || null;
  }

  async listTestPlans(): Promise<Deployment[]> {
    return this.pg.query<Deployment>('SELECT * FROM deployments ORDER BY created_at DESC', []);
  }
  ...
  async createTestCase(data: Omit<Pipeline, 'id' | 'created_at'>): Promise<Pipeline> {
    const id = `tc_${nanoid()}`;
    const now = new Date().toISOString();

    await this.pg.execute(
      `INSERT INTO pipelines (id, plan_id, title, type, steps, expected_result, status, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
  ...
  async listTestCases(planId?: string): Promise<Pipeline[]> {
    if (planId) {
      return this.pg.query<Pipeline>(
        'SELECT * FROM pipelines WHERE plan_id = $1 ORDER BY created_at DESC',
        [planId]
      );
    }
    return this.pg.query<Pipeline>('SELECT * FROM pipelines ORDER BY created_at DESC', []);
  }
  ...
  async createTestScenario(data: Omit<Infrastructure, 'id' | 'created_at'>): Promise<Infrastructure> {
    const id = `ts_${nanoid()}`;
    const now = new Date().toISOString();

    await this.pg.execute(
      `INSERT INTO infrastructure (id, plan_id, title, scenario, tags, created_at)
       VALUES ($1, $2, $3, $4, $5, $6)`,
  ...
  async addTestScenario(planId: string, data: Omit<Infrastructure, 'id' | 'created_at' | 'plan_id'>): Promise<Infrastructure> {
    return this.createTestScenario({ ...data, plan_id: planId });
  }

  async listTestScenarios(planId?: string): Promise<Infrastructure[]> {
    if (planId) {
      return this.pg.query<Infrastructure>(
        'SELECT * FROM infrastructure WHERE plan_id = $1 ORDER BY created_at DESC',
        [planId]
      );
    }
    return this.pg.query<Infrastructure>('SELECT * FROM infrastructure ORDER BY created_at DESC', []);
  }
  ...
  async createBugReport(data: Omit<Incident, 'id' | 'created_at'>): Promise<Incident> {
    const id = `bug_${nanoid()}`;
    const now = new Date().toISOString();

    await this.pg.execute(
      `INSERT INTO incidents (id, title, severity, priority, steps_to_reproduce, expected, actual, environment, status, created_at)
  ...
  async listBugReports(): Promise<Incident[]> {
    return this.pg.query<Incident>('SELECT * FROM incidents ORDER BY created_at DESC', []);
  }
