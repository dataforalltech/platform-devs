#!/usr/bin/env python3
"""
opszilla-mcp - opszilla MCP Service
Port: 7206
"""
import os
import json
...
class PostgresStore:
    def __init__(self, service_name: str = "opszilla"):
...
class OpsZillaStore:
    def __init__(self):
        self.db = PostgresStore('opszilla')

    def list_deployments(self) -> List[Dict[str, Any]]:
        return self.db.query('SELECT * FROM deployments ORDER BY created_at DESC')

app = FastAPI(title="opszilla-mcp", version="1.0.0")
store = OpsZillaStore()
...
TOOLS = {'list_deployments': {'name': 'list_deployments', 'description': 'List deployments', 'inputSchema': {'type': 'object'}}}

@app.post("/mcp/initialize")
async def init(r: MCPRequest) -> MCPResponse:
    return MCPResponse(id=r.id, result={'protocolVersion': '2024-11-05', 'capabilities': {'tools': {}}, 'serverInfo': {'name': 'opszilla-mcp'}})
...
async def call_tool(r: MCPRequest) -> MCPResponse:
    try:
        if r.params.get('name') == 'list_deployments':
            result = store.list_deployments()
        else:
...
if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 7206))
