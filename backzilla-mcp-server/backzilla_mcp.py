#!/usr/bin/env python3
"""
backzilla-mcp - backzilla MCP Service
Port: 7204
"""
import os
import json
...
class PostgresStore:
    def __init__(self, service_name: str = "backzilla"):
...
class BackzillaStore:
    def __init__(self):
        self.db = PostgresStore('backzilla')

    def list_apis(self) -> List[Dict[str, Any]]:
        return self.db.query('SELECT * FROM apis ORDER BY created_at DESC')

app = FastAPI(title="backzilla-mcp", version="1.0.0")
...
TOOLS = {'list_apis': {'name': 'list_apis', 'description': 'List apis', 'inputSchema': {'type': 'object'}}}

@app.post("/mcp/initialize")
async def init(r: MCPRequest) -> MCPResponse:
    return MCPResponse(id=r.id, result={'protocolVersion': '2024-11-05', 'capabilities': {'tools': {}}, 'serverInfo': {'name': 'backzilla-mcp'}})
...
async def call_tool(r: MCPRequest) -> MCPResponse:
    try:
        if r.params.get('name') == 'list_apis':
            result = store.list_apis()
        else:
...
if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 7204))
