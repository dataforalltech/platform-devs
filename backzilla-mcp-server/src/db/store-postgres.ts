...
import * as /home/dev/repos/platform-devs/backzilla-mcp-server/src/db/store-postgres.ts from '/home/dev/repos/platform-devs/backzilla-mcp-server/src/db/store-postgres.ts';
...
// ============================================================================
// BackzillaStore — PostgreSQL Primary
// ============================================================================
...
export class BackzillaStore {
  ...
  constructor() {
    ...
    try {
      this.pg = new ZillaPostgresStore('backzilla', postgresConfig);
      this.logger.info('✅ BackzillaStore: PostgreSQL initialized');
    } catch (error) {
      this.logger.error(`❌ BackzillaStore: Failed to initialize: ${error}`);
      throw error;
    }
  }

  private initializeLogger(): any {
    const logsDir = /home/dev/repos/platform-devs/backzilla-mcp-server/src/db/store-postgres.ts.join(process.env.HOME || '/tmp', '.platform', 'logs');
    if (!fs.existsSync(logsDir)) {
      fs.mkdirSync(logsDir, { recursive: true });
    }

    const logFile = /home/dev/repos/platform-devs/backzilla-mcp-server/src/db/store-postgres.ts.join(logsDir, 'backzilla.log');
  ...
  async createTestPlan(data: Omit<API, 'id' | 'created_at' | 'updated_at'>): Promise<API> {
    const id = `tp_${nanoid()}`;
    const now = new Date().toISOString();

    await this.pg.execute(
      `INSERT INTO apis (id, title, feature, scope, objectives, status, created_at, updated_at)
  ...
  async getTestPlan(id: string): Promise<API | null> {
    const rows = await this.pg.query<API>('SELECT * FROM apis WHERE id = $1', [id]);
    return rows[0] || null;
  }

  async listTestPlans(): Promise<API[]> {
    return this.pg.query<API>('SELECT * FROM apis ORDER BY created_at DESC', []);
  }
  ...
  async createTestCase(data: Omit<Service, 'id' | 'created_at'>): Promise<Service> {
    const id = `tc_${nanoid()}`;
    const now = new Date().toISOString();

    await this.pg.execute(
      `INSERT INTO back_services (id, plan_id, title, type, steps, expected_result, status, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
  ...
  async listTestCases(planId?: string): Promise<Service[]> {
    if (planId) {
      return this.pg.query<Service>(
        'SELECT * FROM back_services WHERE plan_id = $1 ORDER BY created_at DESC',
        [planId]
      );
    }
    return this.pg.query<Service>('SELECT * FROM back_services ORDER BY created_at DESC', []);
  }
  ...
  async createTestScenario(data: Omit<Integration, 'id' | 'created_at'>): Promise<Integration> {
    const id = `ts_${nanoid()}`;
    const now = new Date().toISOString();

    await this.pg.execute(
      `INSERT INTO back_integrations (id, plan_id, title, scenario, tags, created_at)
       VALUES ($1, $2, $3, $4, $5, $6)`,
  ...
  async addTestScenario(planId: string, data: Omit<Integration, 'id' | 'created_at' | 'plan_id'>): Promise<Integration> {
    return this.createTestScenario({ ...data, plan_id: planId });
  }

  async listTestScenarios(planId?: string): Promise<Integration[]> {
    if (planId) {
      return this.pg.query<Integration>(
        'SELECT * FROM back_integrations WHERE plan_id = $1 ORDER BY created_at DESC',
        [planId]
      );
    }
    return this.pg.query<Integration>('SELECT * FROM back_integrations ORDER BY created_at DESC', []);
  }
  ...
  async createChecklist(data: Omit<Workflow, 'id' | 'created_at'>): Promise<Workflow> {
    const id = `cl_${nanoid()}`;
    const now = new Date().toISOString();

    await this.pg.execute(
      `INSERT INTO back_workflows (id, title, items, status, created_at)
       VALUES ($1, $2, $3, $4, $5)`,
