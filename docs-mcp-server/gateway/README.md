# Registro do docs-mcp no MCP Gateway (STD-MCP-001)
...
```
name_microservice = "platform-docs-mcp"
  → namespace = removeprefix("platform-") = "docs-mcp"
  → inner-token audience = "mcp:docs-mcp"
      DEVE == MCP_TWIN_AUDIENCE do sidecar (src/config/settings.py)
```

`MCP_TWIN_AUDIENCE=mcp:docs-mcp` e `name_microservice=platform-docs-mcp` já estão
consistentes. Trave isso no CI com `ci-assert-audience.py` (STD-CICD-001).

## Contrato (resumo)

- `kind=mcp_http`, `call_style=mcp` → envelope `{params:{name,arguments,_meta:{twin_token}}}`.
- `mcp_url=http://platform-docs-mcp:7100` (DNS do container = name_microservice).
- `health_path=/v1/health` (sem token), `tools_list_path=/mcp/tools/list`, `tools_call_path=/mcp/tools/call`.
- `exemptTools=[]` (docs-mcp não expõe tool tokenless; liveness é o próprio `/v1/health`).
  Execução de qualquer tool exige inner token válido (`mcp:docs-mcp`, `jti`).
