...
reg platform-governance-mcp  mcp_http http://platform-governance-mcp:7103    /mcp/tools/list  /mcp/tools/call  mcp  # 8 tools; healthy
# governance-mcp gateia /mcp/tools/list (Twin PEP). O aggregator do platform-mcp manda Authorization: Bearer {apiKey}
# se additional_info tiver apiKey (requer platform-mcp-gateway#9: load_from_db lendo additional_info). apiKey = INTERNAL_API_TOKEN.
GTOK=$(grep '^INTERNAL_API_TOKEN=' /opt/dataforall/deploy/.env | cut -d= -f2-)
M "UPDATE ADMIN_DATAFORALL.GATEWAY_MAPPING SET additional_info=JSON_OBJECT('apiKey','$GTOK') WHERE name_microservice='platform-governance-mcp';" && echo "  + governance-mcp apiKey (Bearer no list)"
