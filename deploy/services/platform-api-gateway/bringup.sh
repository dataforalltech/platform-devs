...
INSERT INTO PLATFORMS (name,url,internal_port,tenant_id,internal_token,domain,db_engine,db_host,db_port,active,excluded)
VALUES ('HML app','https://app.dataforall.tech',8000,'dataforall','${TENANT_TOKEN}','app.dataforall.tech','mysql','tenant-mysql',3306,1,0)
ON DUPLICATE KEY UPDATE domain=VALUES(domain), db_host=VALUES(db_host), active=1, excluded=0;
SQL
echo "seed PLATFORMS:"; docker exec dataforall-admin-mysql mysql -uroot -p"$PW" -N -e "SELECT id,tenant_id,domain FROM ADMIN_DATAFORALL.PLATFORMS;" 2>/dev/null
