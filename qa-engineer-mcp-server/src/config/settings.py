"""Settings do qa-engineer-mcp (sidecar mcp_http, Model C).
...
`qa-engineer-mcp` é uma persona **stateful**: o agente gera o conteúdo (planos,
casos, bugs, quality gates, artefatos de teste) e as tools o **persistem**. A
persistência roda 100% sobre o ORM canônico (`platform_database.orm`), **tenant-
scoped e dual-db**: credencial-zero (ORM-H-12) — o serviço só conhece o `tenant_id`;
a credencial do banco do tenant vem de `ADMIN_DATAFORALL.PLATFORMS` (resolvida pela
lib). Estas Settings expõem os protocolos `DBSettings` (`DB_*`, fallback
compartilhado) e `AdminDBSettings` (`ADMIN_DB_*`, conexão admin que lê PLATFORMS) — o
mesmo objeto é passado a `orm.configure()` no boot.
...
# namespace canônico = name_microservice ('platform-qa-engineer-mcp') menos o
# prefixo 'platform-'. A audiência do inner token DEVE ser exatamente mcp:<namespace>.
NAMESPACE = "qa-engineer-mcp"
...
class QAEngineerSettings(BaseSettings):
    """Settings do qa-engineer-mcp: gateway (Model C) + backend MySQL (ORM canônico, dual-db)."""
    ...
    # ── HTTP sidecar ──────────────────────────────────────────────────────────
    mcp_port: int = Field(default=7124, validation_alias="MCP_PORT")
    ...
    def _resolve_secrets(self) -> QAEngineerSettings:
...
Settings = QAEngineerSettings


@lru_cache(maxsize=1)
def get_settings() -> QAEngineerSettings:
