# Registro do qa-engineer-mcp no MCP Gateway (STD-MCP-001)
...
```
name_microservice = "platform-qa-engineer-mcp"
  → namespace = removeprefix("platform-") = "qa-engineer-mcp"
  → inner-token audience = "mcp:qa-engineer-mcp"
      DEVE == MCP_TWIN_AUDIENCE do sidecar (src/config/settings.py)
```

`MCP_TWIN_AUDIENCE=mcp:qa-engineer-mcp` e `name_microservice=platform-qa-engineer-mcp`
...
- `mcp_url=http://platform-qa-engineer-mcp:7100` (DNS do container = name_microservice).
- `health_path=/v1/health` (sem token), `tools_list_path=/mcp/tools/list`, `tools_call_path=/mcp/tools/call`.
- `exemptTools=[]` (qa-engineer-mcp não tem tool tokenless; a liveness é o `/v1/health`, CI-7);
  execução de toda tool exige inner token válido (`mcp:qa-engineer-mcp`, `jti`).
