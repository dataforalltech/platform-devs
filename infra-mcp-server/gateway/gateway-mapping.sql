-- =============================================================================
-- gateway-mapping.sql — registro DECLARATIVO (pull-based) do infra-mcp no
...
-- prefixo 'platform-' → 'infra-mcp', e audience = 'mcp:infra-mcp'. O PEP do
-- sidecar (MCP_TWIN_AUDIENCE) re-verifica o inner token EXATAMENTE nessa audiência.
-- name_microservice='platform-infra-mcp' ⇒ audience esperada 'mcp:infra-mcp'.
...
VALUES
  ('platform-infra-mcp', NULL, '/v1/health', 30,
   'mcp_http', 'http://platform-infra-mcp:7100', '/mcp/tools/list', '/mcp/tools/call', 'mcp')
...
--     FROM ADMIN_DATAFORALL.GATEWAY_MAPPING WHERE name_microservice='platform-infra-mcp';
--   -- audiência derivada esperada: mcp:infra-mcp
