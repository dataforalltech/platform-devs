/**
 * # Módulo AWS high-mem — Phase 2g
 *
 * Provisiona EC2 r5.2xlarge (8 vCPU, 64 GiB RAM) com Ubuntu 20.04 LTS em
 * subnet pré-existente. Ambiente alvo: cargas intensas em memória (RAG, analytics).
 *
 * Spec "high-mem" exige human_approved=True no request_vm (HUMAN_APPROVAL_REQUIRED_SPECS).
 *
 * Variáveis obrigatórias (via TF_VAR_xxx):
 *   TF_VAR_subnet_id, TF_VAR_vpc_id, TF_VAR_ssh_public_key
 *
 * Idêntico ao aws/cpu-large; apenas instance_type e disk_size diferem.
