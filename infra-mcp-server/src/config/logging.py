...
Deviação justificada do reference (architecture-mcp): este server já emitia um bloco
estruturado ``extras`` (``_log.info(..., extra={"extras": {...}})`` em ~54 call-sites
de src/db e src/server). O ``JsonLogFormatter`` preserva esse campo além dos campos
de correlação canônicos, evitando regressão de observabilidade na migração.

Como o arquivo se chama `logging.py`, os imports usam `from __future__ import
annotations` + import absoluto do stdlib `logging` (o pacote é resolvido por caminho
absoluto, sem colisão com este módulo).
"""
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        for field in _CORRELATION_FIELDS:
            ...
            if value is not None:
                payload[field] = value
        # Bloco estruturado deste server (extra={"extras": {...}}) — preservado.
