"""Fixtures da Suíte Canônica do dev-twin-mcp (§16 — PA-01 / FID-02).
...
from src.db.schema import AGENT_TOKENS_TABLE, ensure_schema
from src.db.store import TokenStore
...
TENANT_A = "devtwin_test_a"
TENANT_B = "devtwin_test_b"
...
def _test_settings(**over: Any) -> DevTwinSettings:
...
async def _make_store(tenant_id: str) -> tuple[TokenStore, Any]:
    ...
    await pool.execute(f"TRUNCATE TABLE {AGENT_TOKENS_TABLE}")
    token = set_tenant_id(tenant_id)
    return TokenStore(TenantSession(pool, tenant_id)), token
...
def clear_session():
    """Sessão em memória limpa entre testes (o SessionManager é singleton por processo)."""
