...
Vault-fallback load_secret (degradação graciosa p/ env) e configure_logging (JSON).
"""
...
def test_enforce_cloud_requires_pg_password():
    s = _base(RUNTIME_ENV="cloud", pg_password="")
...
def test_load_secret_env_when_no_vault(monkeypatch):
    ...
    assert load_secret("dev-twin-mcp/pg_password", "env-pw") == "env-pw"
