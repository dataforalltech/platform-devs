...
// ============================================================================
// ArchZillaStore — PostgreSQL Primary
// ============================================================================
...
export class ArchZillaStore {
  ...
  constructor() {
    ...
    try {
      this.pg = new ZillaPostgresStore('archzilla', postgresConfig);
      this.logger.info('✅ ArchZillaStore: PostgreSQL initialized');
    } catch (error) {
      this.logger.error(`❌ ArchZillaStore: Failed to initialize: ${error}`);
      throw error;
    }
  }
  ...
  async createArchitecture(data: Omit<Architecture, 'id' | 'created_at' | 'updated_at'>): Promise<Architecture> {
    const id = `tp_${nanoid()}`;
    const now = new Date().toISOString();

    await this.pg.execute(
      `INSERT INTO architectures (id, title, feature, scope, objectives, status, created_at, updated_at)
  ...
  async getArchitecture(id: string): Promise<Architecture | null> {
    const rows = await this.pg.query<Architecture>('SELECT * FROM architectures WHERE id = $1', [id]);
    return rows[0] || null;
  }

  async listArchitectures(): Promise<Architecture[]> {
    return this.pg.query<Architecture>('SELECT * FROM architectures ORDER BY created_at DESC', []);
  }
  ...
  async createDecision(data: Omit<Decision, 'id' | 'created_at'>): Promise<Decision> {
    const id = `tc_${nanoid()}`;
    const now = new Date().toISOString();

    await this.pg.execute(
      `INSERT INTO arch_decisions (id, plan_id, title, type, steps, expected_result, status, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
  ...
  async listDecisions(planId?: string): Promise<Decision[]> {
    if (planId) {
      return this.pg.query<Decision>(
        'SELECT * FROM arch_decisions WHERE plan_id = $1 ORDER BY created_at DESC',
        [planId]
      );
    }
    return this.pg.query<Decision>('SELECT * FROM arch_decisions ORDER BY created_at DESC', []);
  }
  ...
  async createDiagram(data: Omit<Diagram, 'id' | 'created_at'>): Promise<Diagram> {
    const id = `ts_${nanoid()}`;
    const now = new Date().toISOString();

    await this.pg.execute(
      `INSERT INTO diagrams (id, plan_id, title, scenario, tags, created_at)
       VALUES ($1, $2, $3, $4, $5, $6)`,
  ...
  async addTestScenario(planId: string, data: Omit<Diagram, 'id' | 'created_at' | 'plan_id'>): Promise<Diagram> {
    return this.createDiagram({ ...data, plan_id: planId });
  }

  async listDiagrams(planId?: string): Promise<Diagram[]> {
    if (planId) {
      return this.pg.query<Diagram>(
        'SELECT * FROM diagrams WHERE plan_id = $1 ORDER BY created_at DESC',
        [planId]
      );
    }
    return this.pg.query<Diagram>('SELECT * FROM diagrams ORDER BY created_at DESC', []);
  }
  ...
  async createQAExecution(data: Omit<QAExecution, 'id' | 'created_at'>): Promise<QAExecution> {
    ...
    await this.pg.execute(
      `INSERT INTO reviews (id, plan_id, status, started_at, completed_at, created_at)
       VALUES ($1, $2, $3, $4, $5, $6)`,
