#!/usr/bin/env python3
"""
archzilla-mcp - archzilla MCP Service
Port: 7203
"""
import os
import json
...
class PostgresStore:
    def __init__(self, service_name: str = "archzilla"):
...
class ArchZillaStore:
    def __init__(self):
        self.db = PostgresStore('archzilla')

    def list_architectures(self) -> List[Dict[str, Any]]:
        return self.db.query('SELECT * FROM architectures ORDER BY created_at DESC')

app = FastAPI(title="archzilla-mcp", version="1.0.0")
store = ArchZillaStore()
...
TOOLS = {'list_architectures': {'name': 'list_architectures', 'description': 'List architectures', 'inputSchema': {'type': 'object'}}}

@app.post("/mcp/initialize")
async def init(r: MCPRequest) -> MCPResponse:
    return MCPResponse(id=r.id, result={'protocolVersion': '2024-11-05', 'capabilities': {'tools': {}}, 'serverInfo': {'name': 'archzilla-mcp'}})
...
async def call_tool(r: MCPRequest) -> MCPResponse:
    try:
        if r.params.get('name') == 'list_architectures':
            result = store.list_architectures()
        else:
...
if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 7203))
