...
class LoadTester:
    ...
    def start_all_devteam(self):
        """Start all DevTeam servers"""
        self.log("Starting all 10 DevTeam...")
        result = subprocess.run(
            ["timeout", "60", "./scripts/start_all_devteam.sh"],
    ...
    def stop_all_devteam(self):
        """Stop all DevTeam servers"""
        self.log("Stopping all DevTeam...")
        subprocess.run(
    ...
    def test_health_checks(self):
        ...
        for devteam_name, port in DEVTEAM:
            ...
            msg = f"  {devteam_name:25} | seq={seq_rps:6.2f} | c5={conc5_rps:6.2f} | c10={conc10_rps:6.2f} req/s"
            self.log(msg)

            self.results[devteam_name] = {
                "seq_rps": seq_rps,
                "conc5_rps": conc5_rps,
    ...
    def test_mcp_tools(self):
        ...
        msg = f"  qa-engineer (create_test_plan) | Sequential: {seq_rps:6.2f} req/s | Concurrent: {conc_rps:6.2f} req/s"
        self.log(msg)
    ...
    def test_aggregate(self):
        ...
        for devteam_name, port in DEVTEAM:
            ...
            if results["times"]:
                ...
                msg = f"  {devteam_name:25} | {rps:7.2f} req/s | Failed: {results['failed']}"
                self.log(msg)
                server_results[devteam_name] = rps
            else:
                self.log(f"  {devteam_name:25} | No data")
    ...
    def run(self):
        """Run all tests"""
        self.log("🚀 Load Testing All 10 DevTeam MCPs")
        self.log("==================================\n")

        self.start_all_devteam()
