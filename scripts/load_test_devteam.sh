#!/bin/bash
# Load testing for all 10 DevTeam MCPs
...
echo "🚀 Load Testing All 10 DevTeam MCPs"
echo "=================================="
echo ""
...
# Start all DevTeam
echo "Starting all 10 DevTeam..."
timeout 60 ./scripts/start_all_devteam.sh > /dev/null 2>&1 || true
sleep 10
...
for devteam_info in "${DEVTEAM[@]}"; do
  ...
  echo "  Testing $devteam_name:$port..." | tee -a "$REPORT_FILE"

  # 50 sequential requests
  ab -q -n 50 -c 1 "http://localhost:$port/health" > "$RESULTS_DIR/${devteam_name}_health_seq.txt" 2>&1
  seq_rps=$(grep "Requests per second:" "$RESULTS_DIR/${devteam_name}_health_seq.txt" | awk '{print $4}')

  # 50 requests, 5 concurrent
  ab -q -n 50 -c 5 "http://localhost:$port/health" > "$RESULTS_DIR/${devteam_name}_health_conc5.txt" 2>&1
  conc5_rps=$(grep "Requests per second:" "$RESULTS_DIR/${devteam_name}_health_conc5.txt" | awk '{print $4}')

  # 50 requests, 10 concurrent
  ab -q -n 50 -c 10 "http://localhost:$port/health" > "$RESULTS_DIR/${devteam_name}_health_conc10.txt" 2>&1
  conc10_rps=$(grep "Requests per second:" "$RESULTS_DIR/${devteam_name}_health_conc10.txt" | awk '{print $4}')
...
# Test 2: MCP tool calls (create_test_plan on qa-engineer)
...
echo "  Testing qa-engineer MCP tool calls..."  | tee -a "$REPORT_FILE"

# 20 sequential MCP calls
ab -q -n 20 -c 1 -p /tmp/mcp_payload.json -T "application/json" \
  "http://localhost:7201/mcp/tools/call" > "$RESULTS_DIR/qa-engineer_mcp_seq.txt" 2>&1
mcp_seq_rps=$(grep "Requests per second:" "$RESULTS_DIR/qa-engineer_mcp_seq.txt" | awk '{print $4}')

# 20 concurrent MCP calls
ab -q -n 20 -c 5 -p /tmp/mcp_payload.json -T "application/json" \
  "http://localhost:7201/mcp/tools/call" > "$RESULTS_DIR/qa-engineer_mcp_conc.txt" 2>&1
mcp_conc_rps=$(grep "Requests per second:" "$RESULTS_DIR/qa-engineer_mcp_conc.txt" | awk '{print $4}')
...
for devteam_info in "${DEVTEAM[@]}"; do
  ...
  ab -q -n 100 -c 20 "http://localhost:$port/health" > "$RESULTS_DIR/${devteam_name}_aggregate.txt" 2>&1

  requests=$(grep "Requests per second:" "$RESULTS_DIR/${devteam_name}_aggregate.txt" | awk '{print $4}')
  failed=$(grep "Failed requests:" "$RESULTS_DIR/${devteam_name}_aggregate.txt" | awk '{print $3}')
  ...
  echo "  $devteam_name: $(printf "%.2f" $requests) req/s | Failed: $failed" | tee -a "$REPORT_FILE"
done
...
# Stop all DevTeam
echo ""
echo "Stopping all DevTeam..."
./scripts/stop_all_devteam.sh > /dev/null 2>&1 || true
