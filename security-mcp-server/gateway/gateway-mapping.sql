-- =============================================================================
-- gateway-mapping.sql — registro DECLARATIVO (pull-based) do security-mcp no
...
-- prefixo 'platform-' → 'security-mcp', e audience = 'mcp:security-mcp'. O PEP do
-- sidecar (MCP_TWIN_AUDIENCE) re-verifica o inner token EXATAMENTE nessa audiência.
-- name_microservice='platform-security-mcp' ⇒ audience esperada 'mcp:security-mcp'.
...
VALUES
  ('platform-security-mcp', NULL, '/v1/health', 30,
   'mcp_http', 'http://platform-security-mcp:7100', '/mcp/tools/list', '/mcp/tools/call', 'mcp')
...
--     FROM ADMIN_DATAFORALL.GATEWAY_MAPPING WHERE name_microservice='platform-security-mcp';
--   -- audiência derivada esperada: mcp:security-mcp
