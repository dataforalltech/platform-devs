# Registro do security-mcp no MCP Gateway (STD-MCP-001)
...
```
name_microservice = "platform-security-mcp"
  → namespace = removeprefix("platform-") = "security-mcp"
  → inner-token audience = "mcp:security-mcp"
      DEVE == MCP_TWIN_AUDIENCE do sidecar (src/config/settings.py)
```

`MCP_TWIN_AUDIENCE=mcp:security-mcp` e `name_microservice=platform-security-mcp`
...
- `mcp_url=http://platform-security-mcp:7100` (DNS do container = name_microservice).
- `health_path=/v1/health` (sem token), `tools_list_path=/mcp/tools/list`, `tools_call_path=/mcp/tools/call`.
- `exemptTools=["status"]` (tokenless); execução das demais exige inner token válido (`mcp:security-mcp`, `jti`).
