"""Settings do security-mcp (sidecar mcp_http, Model C).
...
`security-mcp` é uma persona **stateful**: o agente gera o conteúdo (modelos de
ameaças, controles, avaliações CVSS, artefatos de segurança) e as tools o **persistem**.
...
# namespace canônico = name_microservice ('platform-security-mcp') menos o prefixo
# 'platform-'. A audiência do inner token DEVE ser exatamente mcp:<namespace>.
NAMESPACE = "security-mcp"
...
class SecuritySettings(BaseSettings):
    """Settings do security-mcp: gateway (Model C) + backend MySQL (ORM canônico, dual-db)."""
    ...
    def _resolve_secrets(self) -> SecuritySettings:
