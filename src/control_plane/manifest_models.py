...
class MCPManifest(StrictModel):
    ...
    def validate_lifecycle(self) -> "MCPManifest":
        ...
        if self.status == "planned":
            ...
            if any(enabled):
                raise ValueError("planned manifests cannot be enabled on runtime surfaces")
            if self.tool_catalog or self.tool_contracts:
                raise ValueError("planned manifests cannot publish tools or tool contracts")
        if not executable and (self.gateway.enabled or self.registry.enabled):
            raise ValueError("only active/experimental manifests may enter gateway or registry")
        if executable and self.runtime.mode == "local":
            if not (self.runtime.command and self.runtime.cwd and self.runtime.service_name):
                raise ValueError("local executable runtime requires command, cwd and service_name")
        if executable and self.runtime.mode == "external" and not self.runtime.base_url_env:
        ...
        if executable and self.runtime.exposes_tools:
            if not all(
                ...
                raise ValueError("executable tool providers require auth, tenant, audit, redaction and signing")
            if self.security.policy_mode != "fail_closed":
                raise ValueError("executable tool providers must use fail_closed policy")
        return self
...
class ToolRisk(StrictModel):
    ...
    blast_radius: Literal["none", "workspace", "service", "environment", "tenant", "global"]
...
class ToolContract(StrictModel):
    ...
    def validate_governance(self) -> "ToolContract":
        ...
        if self.risk.level in {"high", "critical"} and self.risk.approval_required == "none":
            raise ValueError("high/critical tools require approval")
        if self.secret_reference_only:
            forbidden = re.compile(r"(^|_)(password|secret|private_key|credential|token|value)($|_)")
            ...
            for schema in (self.input_schema, self.output_schema):
                for key in _schema_property_names(schema):
                    if forbidden.search(key.lower()) and not key.lower().endswith(allowed_suffixes):
                        raise ValueError(f"secret-bearing field is forbidden; use an opaque reference: {key}")
        return self
