"""Fixtures da Suíte Canônica do qa-mcp (§16 — PA-01 / FID-02).
...
As ferramentas EXTERNAS de QA (pytest/ruff/bandit/mypy/radon via subprocess, Playwright,
httpx) seguem mockadas nos testes de tool (FID-01 permite duplo de serviço externo); o
banco é que nunca é mockado.
...
def _test_settings(**over: Any) -> QASettings:
    kw: dict[str, Any] = dict(
        ...
        http_timeout=5.0,
        screenshots_dir="/tmp/qa-test-screenshots",  # noqa: S108
        baselines_dir="/tmp/qa-test-baselines",  # noqa: S108
    )
    kw.update(over)
    return QASettings(**kw)
...
async def _make_store(tenant_id: str) -> tuple[QAStore, Any]:
    ...
    await pool.execute("TRUNCATE TABLE test_runs")
    token = set_tenant_id(tenant_id)
    return QAStore(TenantSession(pool, tenant_id)), token
...
def settings() -> QASettings:
    """Settings de teste para as tools (dirs/thresholds); tools sobrescrevem dirs via
    model_copy quando precisam de um tmp_path real."""
