-- =============================================================================
-- gateway-mapping.sql — registro DECLARATIVO (pull-based) do qa-mcp no
...
-- prefixo 'platform-' → 'qa-mcp', e audience = 'mcp:qa-mcp'. O PEP do sidecar
-- (MCP_TWIN_AUDIENCE) re-verifica o inner token EXATAMENTE nessa audiência.
-- name_microservice='platform-qa-mcp' ⇒ audience esperada 'mcp:qa-mcp'.
...
VALUES
  ('platform-qa-mcp', NULL, '/v1/health', 30,
   'mcp_http', 'http://platform-qa-mcp:7100', '/mcp/tools/list', '/mcp/tools/call', 'mcp')
...
--     FROM ADMIN_DATAFORALL.GATEWAY_MAPPING WHERE name_microservice='platform-qa-mcp';
--   -- audiência derivada esperada: mcp:qa-mcp
