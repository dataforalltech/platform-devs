#!/usr/bin/env python3
"""
Gera arquivos store-postgres.ts para todos os DevTeam
Usa qa-engineer como template e faz adaptações por DevTeam

Usage:
    python3 generate_devteam_stores.py
"""

from pathlib import Path
import re

# Definição de cada DevTeam: nome, tabelas, caminho
DEVTEAM = {
    'security': {
        'path': '/home/dev/repos/platform-devs/security-mcp-server/src/db/store-postgres.ts',
        'tables': {
    ...
    'architecture': {
        'path': '/home/dev/repos/platform-devs/architecture-mcp-server/src/db/store-postgres.ts',
        'tables': {
    ...
    'backend': {
        'path': '/home/dev/repos/platform-devs/backend-mcp-server/src/db/store-postgres.ts',
        'tables': {
    ...
    'frontend': {
        'path': '/home/dev/repos/platform-devs/frontend-pixelfera-mcp-server/src/db/store-postgres.ts',
        'tables': {
    ...
    'devops': {
        'path': '/home/dev/repos/platform-devs/devops-mcp-server/src/db/store-postgres.ts',
        'tables': {
    ...
    'product-owner': {
        'path': '/home/dev/repos/platform-devs/product-owner-mcp-server/src/db/store-postgres.ts',
        'tables': {
    ...
    'product-manager': {
        'path': '/home/dev/repos/platform-devs/product-manager-mcp-server/src/db/store-postgres.ts',
        'tables': {
...
// ============================================================================
// {DEVTEAM_UPPER}Store — PostgreSQL Primary
// ============================================================================

export class {DEVTEAM_UPPER}Store {{
  private pg: DevTeamPostgresStore;
  private logger: any;
  ...
  constructor() {{
    ...
    try {{
      this.pg = new DevTeamPostgresStore('{devteam}', postgresConfig);
      this.logger.info('✅ {DEVTEAM_UPPER}Store: PostgreSQL initialized');
    }} catch (error) {{
      this.logger.error(`❌ {DEVTEAM_UPPER}Store: Failed to initialize: ${{error}}`);
      throw error;
    }}
  }}
  ...
  private initializeLogger(): any {{
    ...
    const logFile = path.join(logsDir, '{devteam}.log');
...
def generate_methods(devteam: str, tables: dict) -> str:
    """Generate CRUD methods for a DevTeam"""
    methods = []
...
def main():
    print("🚀 Gerando store-postgres.ts para todos os DevTeam...\n")

    for devteam_name, config in DEVTEAM.items():
        print(f"📝 Gerando {devteam_name}...")
        ...
        content = TEMPLATE.format(
            ...
            DEVTEAM_UPPER=devteam_name.capitalize(),
            devteam=devteam_name,
            METHODS=methods_str
        )
    ...
    print("   Para implementação completa, copie os padrões de qa-engineer-mcp-server/src/db/store-postgres.ts")
