...
def test_formatter_emits_json_with_service_and_msg():
    ...
    assert payload["service"] == "session-mcp"
    assert payload["level"] == "INFO"
    assert payload["logger"] == "session.test"
...
def test_formatter_includes_correlation_fields_when_present():
    payload = json.loads(
        JsonLogFormatter().format(_record(tenant_id="T-1", tool="start_session", trace_id="abc"))
    )
    assert payload["tenant_id"] == "T-1"
    assert payload["tool"] == "start_session"
...
def test_configure_logging_installs_single_json_handler():
    ...
    try:
        configure_logging(SessionSettings(MCP_SERVICE_LOG_LEVEL="warning"))
