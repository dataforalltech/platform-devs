...
class TestFailTask:
    def test_marks_failed_with_reason(self, store, active_session):
        sid = active_session["id"]
        task = add_task(store, session_id=sid, title="t")
        result = fail_task(store, task_id=task["id"], actor=_ACTOR_AGENT, reason="biblioteca não suporta")
...
class TestRejectDeferSupersede:
    def test_reject_with_reason(self, store):
        ...
        result = reject_suggestion(store, suggestion_id=s["id"], actor=_ACTOR_HUMAN, reason="duplicada")
...
class TestDecisionsAudit:
    ...
    def test_filter_by_actor(self, store, active_session):
        ...
        human_decisions = list_decisions_tool(store, actor_type="human", actor_id="test@dataforall.tech")
