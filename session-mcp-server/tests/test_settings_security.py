...
from src.config.settings import SessionSettings, load_secret


def _base(**over: object) -> SessionSettings:
    kw: dict[str, object] = {
...
def test_enforce_ok_local_defaults():
    SessionSettings().enforce_security_invariants()  # local não exige JWKS/admin
...
def test_enforce_bad_audience_raises():
    with pytest.raises(RuntimeError, match="STD-SEC-006"):
        _base(MCP_TWIN_AUDIENCE="session-mcp").enforce_security_invariants()
...
def test_load_secret_no_vault_returns_env(monkeypatch):
    ...
    assert load_secret("session-mcp/db_password", "from-env") == "from-env"


def test_load_secret_vault_import_failure_degrades(monkeypatch):
    monkeypatch.setenv("VAULT_ADDR", "http://vault.local:8200")
    assert load_secret("session-mcp/db_password", "fallback") == "fallback"
...
def test_load_secret_vault_success(monkeypatch):
    ...
    assert load_secret("session-mcp/db_password", "fallback") == "from-vault"
...
def test_settings_resolves_secrets_via_vault(monkeypatch):
    ...
    s = SessionSettings(DB_PASSWORD="ignored", ADMIN_DB_PASSWORD="ignored")
