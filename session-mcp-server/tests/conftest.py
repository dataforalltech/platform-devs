"""Fixtures da Suíte Canônica do session-mcp (§16 — PA-01 / FID-02).
...
TENANT_A = "session_test_a"
TENANT_B = "session_test_b"

# Todas as tabelas do domínio — TRUNCATE por teste (estado limpo, idempotente e rápido).
_ALL_TABLES = (
    "decisions",
    "suggestions",
    "session_services",
...
def _test_settings(**over: Any) -> SessionSettings:
...
async def _make_store(tenant_id: str) -> tuple[SessionStore, Any]:
    ...
    return SessionStore(TenantSession(pool, tenant_id)), token
...
@pytest_asyncio.fixture
async def active_session(store_a: SessionStore):
    """Uma sessão ativa criada no tenant A (com branch sugerida), via start_session."""
