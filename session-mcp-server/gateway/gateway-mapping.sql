-- =============================================================================
-- gateway-mapping.sql — registro DECLARATIVO (pull-based) do session-mcp no
...
-- prefixo 'platform-' → 'session-mcp', e audience = 'mcp:session-mcp'. O PEP do
-- sidecar (MCP_TWIN_AUDIENCE) re-verifica o inner token EXATAMENTE nessa audiência.
-- name_microservice='platform-session-mcp' ⇒ audience esperada 'mcp:session-mcp'.
...
VALUES
  ('platform-session-mcp', NULL, '/v1/health', 30,
   'mcp_http', 'http://platform-session-mcp:7100', '/mcp/tools/list', '/mcp/tools/call', 'mcp')
...
--     FROM ADMIN_DATAFORALL.GATEWAY_MAPPING WHERE name_microservice='platform-session-mcp';
--   -- audiência derivada esperada: mcp:session-mcp
