...
`session-mcp` é uma persona **stateful** (mantém sessões/tasks/sugestões/decisões de
trabalho Claude Code). A persistência roda 100% sobre o ORM canônico
...
class SessionSettings(BaseSettings):
    """Settings do session-mcp: gateway (Model C) + backend dual-db (ORM canônico)."""
    ...
    max_active_sessions: int = Field(default=50, validation_alias="SESSION_MAX_ACTIVE_SESSIONS")
    default_list_limit: int = Field(default=20, validation_alias="SESSION_DEFAULT_LIST_LIMIT")
    default_base_branch: str = Field(
        default="develop",
        validation_alias="SESSION_DEFAULT_BASE_BRANCH",
    ...
    def _resolve_secrets(self) -> SessionSettings:
