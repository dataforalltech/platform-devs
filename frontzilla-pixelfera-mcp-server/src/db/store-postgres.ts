...
import * as /home/dev/repos/platform-devs/frontzilla-pixelfera-mcp-server/src/db/store-postgres.ts from '/home/dev/repos/platform-devs/frontzilla-pixelfera-mcp-server/src/db/store-postgres.ts';
...
// ============================================================================
// FrontzillaStore — PostgreSQL Primary
// ============================================================================

export class FrontzillaStore {
  ...
  constructor() {
    ...
    try {
      this.pg = new ZillaPostgresStore('frontzilla', postgresConfig);
      this.logger.info('✅ FrontzillaStore: PostgreSQL initialized');
    } catch (error) {
      this.logger.error(`❌ FrontzillaStore: Failed to initialize: ${error}`);
      throw error;
    }
  }

  private initializeLogger(): any {
    const logsDir = /home/dev/repos/platform-devs/frontzilla-pixelfera-mcp-server/src/db/store-postgres.ts.join(process.env.HOME || '/tmp', '.platform', 'logs');
    if (!fs.existsSync(logsDir)) {
      fs.mkdirSync(logsDir, { recursive: true });
    }

    const logFile = /home/dev/repos/platform-devs/frontzilla-pixelfera-mcp-server/src/db/store-postgres.ts.join(logsDir, 'frontzilla.log');
  ...
  async createTestPlan(data: Omit<Feature, 'id' | 'created_at' | 'updated_at'>): Promise<Feature> {
    ...
    await this.pg.execute(
      `INSERT INTO front_features (id, title, feature, scope, objectives, status, created_at, updated_at)
  ...
  async getTestPlan(id: string): Promise<Feature | null> {
    const rows = await this.pg.query<Feature>('SELECT * FROM front_features WHERE id = $1', [id]);
    return rows[0] || null;
  }

  async listTestPlans(): Promise<Feature[]> {
    return this.pg.query<Feature>('SELECT * FROM front_features ORDER BY created_at DESC', []);
  }
  ...
  async createTestCase(data: Omit<Component, 'id' | 'created_at'>): Promise<Component> {
    const id = `tc_${nanoid()}`;
    const now = new Date().toISOString();

    await this.pg.execute(
      `INSERT INTO components (id, plan_id, title, type, steps, expected_result, status, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
  ...
  async listTestCases(planId?: string): Promise<Component[]> {
    if (planId) {
      return this.pg.query<Component>(
        'SELECT * FROM components WHERE plan_id = $1 ORDER BY created_at DESC',
        [planId]
      );
    }
    return this.pg.query<Component>('SELECT * FROM components ORDER BY created_at DESC', []);
  }
  ...
  async createTestScenario(data: Omit<DesignToken, 'id' | 'created_at'>): Promise<DesignToken> {
    const id = `ts_${nanoid()}`;
    const now = new Date().toISOString();

    await this.pg.execute(
      `INSERT INTO design_tokens (id, plan_id, title, scenario, tags, created_at)
       VALUES ($1, $2, $3, $4, $5, $6)`,
  ...
  async addTestScenario(planId: string, data: Omit<DesignToken, 'id' | 'created_at' | 'plan_id'>): Promise<DesignToken> {
    return this.createTestScenario({ ...data, plan_id: planId });
  }

  async listTestScenarios(planId?: string): Promise<DesignToken[]> {
    if (planId) {
      return this.pg.query<DesignToken>(
        'SELECT * FROM design_tokens WHERE plan_id = $1 ORDER BY created_at DESC',
        [planId]
      );
    }
    return this.pg.query<DesignToken>('SELECT * FROM design_tokens ORDER BY created_at DESC', []);
  }
  ...
  async createChecklist(data: Omit<Workflow, 'id' | 'created_at'>): Promise<Workflow> {
    ...
    await this.pg.execute(
      `INSERT INTO front_workflows (id, title, items, status, created_at)
       VALUES ($1, $2, $3, $4, $5)`,
