#!/usr/bin/env python3
"""
frontzilla-mcp - frontzilla MCP Service
Port: 7205
"""
import os
import json
...
class PostgresStore:
    def __init__(self, service_name: str = "frontzilla"):
...
class FrontzillaStore:
    def __init__(self):
        self.db = PostgresStore('frontzilla')

    def list_front_features(self) -> List[Dict[str, Any]]:
        return self.db.query('SELECT * FROM front_features ORDER BY created_at DESC')

app = FastAPI(title="frontzilla-mcp", version="1.0.0")
store = FrontzillaStore()
...
TOOLS = {'list_front_features': {'name': 'list_front_features', 'description': 'List front_features', 'inputSchema': {'type': 'object'}}}

@app.post("/mcp/initialize")
async def init(r: MCPRequest) -> MCPResponse:
    return MCPResponse(id=r.id, result={'protocolVersion': '2024-11-05', 'capabilities': {'tools': {}}, 'serverInfo': {'name': 'frontzilla-mcp'}})
...
async def call_tool(r: MCPRequest) -> MCPResponse:
    try:
        if r.params.get('name') == 'list_front_features':
            result = store.list_front_features()
        else:
...
if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 7205))
