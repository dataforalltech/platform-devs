"""Settings do backend-mcp (sidecar mcp_http, Model C).
...
`backend-mcp` é uma persona **stateful**: o agente gera o conteúdo (contratos de API,
schemas de banco, políticas de auth, artefatos de código, reviews) e as tools o
...
# namespace canônico = name_microservice ('platform-backend-mcp') menos o prefixo
...
class BackendSettings(BaseSettings):
    """Settings do backend-mcp: gateway (Model C) + backend MySQL (ORM canônico, dual-db)."""
    ...
    def _resolve_secrets(self) -> BackendSettings:
