-- =============================================================================
-- gateway-mapping.sql — registro DECLARATIVO (pull-based) do backend-mcp no
...
-- prefixo 'platform-' → 'backend-mcp', e audience = 'mcp:backend-mcp'. O PEP do
-- sidecar (MCP_TWIN_AUDIENCE) re-verifica o inner token EXATAMENTE nessa audiência.
-- name_microservice='platform-backend-mcp' ⇒ audience esperada 'mcp:backend-mcp'.
...
VALUES
  ('platform-backend-mcp', NULL, '/v1/health', 30,
   'mcp_http', 'http://platform-backend-mcp:7100', '/mcp/tools/list', '/mcp/tools/call', 'mcp')
...
--     FROM ADMIN_DATAFORALL.GATEWAY_MAPPING WHERE name_microservice='platform-backend-mcp';
--   -- audiência derivada esperada: mcp:backend-mcp
